...
Each Unreleased or version block may only have one heading per section type, for example a single
`### Features` block. Duplicate section headings are rejected by `make check-changelog`.
