...
Never open a duplicate: a live GitHub search precedes every create. If an ignored entry no longer reproduces, say so in the console summary — a human can prune the list. If an open `relay-audit:` issue's mismatch no longer reproduces this run, the reverse check (Procedure step 6) surfaces it for a human to confirm and close — the audit never closes issues itself.
...
5. For each mismatch that survives the cross-SDK gate, run the GitHub-issue pipeline (see "GitHub issues"): search for an existing `relay-audit:` issue on fingerprint; edit its description if found, else create a new one.
6. **Reverse check (no-longer-reproduces).** List all **open** `relay-audit:` issues (`gh issue list --repo getsentry/sentry-cocoa --search "relay-audit: in:title" --state open --limit 1000`; same pagination guard as dedup — if the result equals the limit, page before trusting it) and diff them against this run's finding set **by fingerprint**. When matching, count **every mismatch observed this run** — reported findings, findings **ignored** at Procedure step 3, and findings **dropped as `all-sdks-agree`** at the cross-SDK gate. All three still reproduce; only the pipeline's disposition differs, so any of them keeps its open issue legitimately open. Match on the raw mismatch set from Procedure step 2, not just the filed subset — enumerating "reported + ignored" (and forgetting a category) is exactly what mis-flags an issue. Any open issue with **no** matching mismatch this run is a candidate no-longer-reproduces: **do not close it** (a run may miss a finding for benign reasons — a moved file, a flaky peer search); instead list it in the console summary under "no longer reproduces — verify & close" with its number, so a human confirms and closes. This is the mechanism behind the Model's promise; without it a fixed mismatch's issue would linger forever.
...
3. **Match found** → branch on the matched issue's `state`:
   ...
   - **closed** → a closed issue whose finding still reproduces is a **possible regression**, but the audit must not presume _why_ a human closed it (fixed-and-regressed vs. closed as invalid/duplicate/won't-fix-without-ignore-entry). So **do not reopen it automatically** — that would fight a human's decision and churn the tracker on every run. Instead **surface it for a human**: post one `gh issue comment <number>` — _"This finding reproduces again as of `<SHORT_SHA>` (<DATE>). If it regressed, reopen; if this issue was closed as invalid/duplicate, add it to the ignore list so the audit stops resurfacing it. — relay-conformance-audit"_ — and list it in the console summary under `closed but still reproduces — reopen or ignore: #<n>`. Comment **at most once per issue**: skip the comment if the most recent audit comment already says the same thing (avoid per-run noise). Do not create a new issue (that would duplicate the closed one), do not edit the closed issue's body, do not reopen.
...
**Proof of the correct way (peer SDK):** <one line naming a peer SDK that implements this correctly, with an inline `blob/`-pinned link to the exact peer line — e.g. sentry-java at [`Baggage.java#L41`](https://github.com/getsentry/sentry-java/blob/<peer-sha>/...#L41). Include for `mixed`/`cocoa-only` (link a Relay-matching peer) and `ecosystem-divergent` (link a third-value peer). Omit for `inconclusive` (no peer located) and for an escape-hatched `all-sdks-agree` (no peer disagrees with Cocoa — instead write "Proof: none — every peer matches Cocoa; escalated as a likely spec/Relay-wide issue" and link the Relay source you re-read).> Cross-SDK verdict: <verdict + one-clause summary>.
...
- `closed but still reproduces — reopen or ignore: #<n>, #<n>` — closed matches the audit commented on (from Dedup step 3), for a human to reopen or add to the ignore list; omit if none;
...
Nothing needs action → say so, still print the drop count, the reverse-check line, and the coverage line.
