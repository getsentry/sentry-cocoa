...
3. Classify each mismatch against `references/findings.md` on fingerprint (`area + file + normalized summary` — never line numbers/severity; those vary between runs): listed and ignore-scenario still holds → **ignored**, skip; listed but scenario **no longer holds** → **needs action (expired-ignore)**, carrying that flag and the entry's `(relay-audit #<n>)` back-reference (Dedup step 3 reopens its closed issue); not listed → **needs action**.
