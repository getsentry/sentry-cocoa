...
6. **Reverse check (no-longer-reproduces).** List all **open** `relay-audit:` issues (`gh issue list --repo getsentry/sentry-cocoa --search "relay-audit: in:title" --state open`) and diff them against this run's surviving finding set by fingerprint. Any open issue with **no** matching finding this run is a candidate no-longer-reproduces: **do not close it** (a run may miss a finding for benign reasons — a moved file, a flaky peer search); instead list it in the console summary under "no longer reproduces — verify & close" with its number, so a human confirms and closes. This is the mechanism behind the Model's promise; without it a fixed mismatch's issue would linger forever.
