...
- **Needs action** — everything else, unless cross-SDK corroboration drops it. The skill searches GitHub for an existing `relay-audit:` issue matching the mismatch (by fingerprint) and:
  ...
  - **closed match** → if the finding is an **expired-ignore** (the audit's own ignore PR closed it and the scenario has lapsed) it **reopens** the issue — the one safe auto-reopen, since the audit itself did the closing. Otherwise it **comments once** flagging that the finding reproduces again (reopen if regressed, or add to the ignore list if closed as invalid/duplicate) and surfaces it in the console summary — it does **not** auto-reopen or create a duplicate; a human decides.
...
3. Classify each mismatch. First check `references/findings.md` on fingerprint (`area + file + normalized summary` — never line numbers, never severity: severity ratings vary between runs, fingerprints are the stable key): if listed and the ignore-scenario still holds → **ignored**, skip. If listed but the ignore-scenario **no longer holds** → **needs action (expired-ignore)** — carry that flag plus the entry's `(relay-audit #<n>)` back-reference, because its issue was closed by the ignore PR and must be **reopened**, not just commented (see Dedup step 3). Not listed → **needs action**.
...
5. For each mismatch that survives the cross-SDK gate, run the GitHub-issue pipeline (see "GitHub issues"): search for an existing `relay-audit:` issue on fingerprint, then per Dedup step 3 — **open match** → edit its description; **closed match + expired-ignore** → reopen + edit (the audit closed it via its own ignore PR); **closed match, any other reason** → comment once (do not reopen/edit/recreate); **no match** → create a new issue.
...
Each needs-action mismatch maps to exactly one `relay-audit:` GitHub issue on `getsentry/sentry-cocoa`: a new issue when none matches, an edit when an **open** issue matches, a reopen+edit when a **closed** issue matches an **expired-ignore** finding (the audit's own ignore PR closed it), or a single flag-comment when a **closed** issue matches for any other reason (see Dedup step 3 for the full branch). **Never open a duplicate; the only closed issue the audit reopens is an expired-ignore one it closed itself — never reopen or edit a human-closed issue.**

### Dedup (do this before every create)

1. `gh issue list --repo getsentry/sentry-cocoa --search "relay-audit: in:title" --state all --limit 1000 --json number,title,body,state,comments,updatedAt` — search **open and closed** issues; `state` is required to branch open vs closed in step 3, `comments` is required so the closed-match path can skip re-posting when its last audit comment already flagged this (per-run de-dup — without it the skill can't tell it already commented), and `updatedAt` is required to pick the most-recently-updated issue when several match one fingerprint (the default search order is relevance, not recency, so sort on `updatedAt` yourself). The list is scoped to the `relay-audit:` title prefix, so `1000` covers the audit's own issues for the foreseeable future; **if the result ever returns exactly the limit, the cap was hit — paginate (bump `--limit` or page with the Search API) before trusting a "no match", or a duplicate could slip past the 1000th result.**
...
- `closed but still reproduces — reopen or ignore: #<n>, #<n>` — **every** closed match whose finding reproduced this run (from Dedup step 3), except expired-ignore reopens (those show as `reopened (ignore expired)` above); regardless of whether a comment was posted or skipped-as-duplicate this run; for a human to reopen or add to the ignore list. This is the standing signal, so it must not disappear after the one-time comment; omit only when there are genuinely none this run;
