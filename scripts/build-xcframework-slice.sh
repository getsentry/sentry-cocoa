...
if [ "$sdk" = "maccatalyst" ]; then
    ...
    set -o pipefail && NSUnbufferedIO=YES xcodebuild \
        ...
        GCC_PREPROCESSOR_DEFINITIONS="\$(inherited) SENTRY_PREBUILT_FRAMEWORK=1" 2>&1 | tee "${slice_id}.maccatalyst.log" | xcbeautify
    ...
    set -o pipefail && NSUnbufferedIO=YES xcodebuild archive \
        ...
        GCC_GENERATE_DEBUGGING_SYMBOLS="$GCC_GENERATE_DEBUGGING_SYMBOLS" \
        OTHER_LDFLAGS="$OTHER_LDFLAGS" \
        GCC_PREPROCESSOR_DEFINITIONS="\$(inherited) SENTRY_PREBUILT_FRAMEWORK=1" 2>&1 | tee "${slice_id}.log" | xcbeautify
fi
