...
TESTAPPS_DIR="$REPO_ROOT/TestApps"

SPECS=()
PRODUCT=""
WITH=""

# Binary XCFramework test apps cannot enable V10.
SKIP_SPECS=(
...
collect_specs() {
    ...
    while IFS= read -r -d '' spec; do
        ...
        printf '%s\0' "$spec"
    done < <(find "$TESTAPPS_DIR" -name '*.yml' ! -path '*/Shared/*' -print0)
}
...
if [[ ${#SPECS[@]} -eq 0 ]]; then
    log_warning "No XcodeGen specs with local packages found under TestApps/"
    exit 0
fi
