...
#
# Both variants bundle the three static archives (Sentry, SentryObjCCompat,
# SentryObjC) into a single binary so downstream consumers only have to link
# one framework, and neither requires Clang modules.
#
...
for sdk in "${sdks[@]}"; do
    ...
    compat_archive="${ARCHIVE_BASE}/SentryObjCCompat/${sdk}.xcarchive/Products/Library/Frameworks/SentryObjCCompat.framework"
    ...
    if [ "$sdk" = "macosx" ] || [ "$sdk" = "maccatalyst" ]; then
        ...
        compat_a="${compat_archive}/Versions/A/SentryObjCCompat"
        objc_a="${objc_archive}/Versions/A/SentryObjC"
    else
        sentry_a="${sentry_archive}/Sentry"
        compat_a="${compat_archive}/SentryObjCCompat"
        objc_a="${objc_archive}/SentryObjC"
    fi

    for lib in "$sentry_a" "$compat_a" "$objc_a"; do
        if [ ! -f "$lib" ]; then
    ...
    libtool -static "$sentry_a" "$compat_a" "$objc_a" -o "$combined_a"
