...
if [ $PLATFORM == "iOS" -a $OS == "12.4" ]; then
    ...
    SENTRY_XCODE_VERSION=$XCODE_VERSION SENTRY_PLATFORM=$PLATFORM env NSUnbufferedIO=YES xcodebuild -workspace Sentry.xcworkspace -scheme Sentry -configuration Test \
    ...
    SENTRY_XCODE_VERSION=$XCODE_VERSION SENTRY_PLATFORM=$PLATFORM env NSUnbufferedIO=YES xcodebuild -workspace Sentry.xcworkspace -scheme Sentry -configuration TestCI \
        GCC_GENERATE_TEST_COVERAGE_FILES=YES GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=YES -destination "$DESTINATION" \
        test | xcpretty -t && exit ${PIPESTATUS[0]}
fi
