...
SENTRY_FRAMEWORK_PLATFORM=$PLATFORM
if [[ $SENTRY_FRAMEWORK_PLATFORM = "Catalyst" ]]; then
    SENTRY_FRAMEWORK_PLATFORM="iOS"
fi
...
if [ $PLATFORM == "iOS" -a $OS == "12.4" ]; then
    ...
    env NSUnbufferedIO=YES xcodebuild -project Sentry.xcodeproj -scheme Sentry_$SENTRY_FRAMEWORK_PLATFORM -configuration $CONFIGURATION \
        ...
        -skip-testing:"SentryTests_iOS/SentryNetworkTrackerIntegrationTests/testGetRequest_SpanCreatedAndTraceHeaderAdded" \
        -skip-testing:"SentryTests_iOS/SentrySDKTests/testMemoryFootprintOfAddingBreadcrumbs" \
        -skip-testing:"SentryTests_iOS/SentrySDKTests/testMemoryFootprintOfTransactions" \
