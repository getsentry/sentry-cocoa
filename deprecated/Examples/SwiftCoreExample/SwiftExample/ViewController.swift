...
class ViewController: UIViewController, SentryClientUserFeedbackDelegate {
	...
	override func viewDidLoad() {
		...
		// Step 3: Set any user or global information to be sent up with every exception/message
    ...
    func userFeedbackReady() {
        if let viewControllers = SentryClient.shared?.userFeedbackControllers(), self.presentedViewController == nil {
            present(viewControllers.navigationController, animated: true, completion: nil)
        }
    }
    ...
    @IBAction func onClickBreadcrumbMessage(_ sender: AnyObject) {
	...
	@IBAction func onClickBreak(_ sender: AnyObject) {
		SentryClient.shared?.breadcrumbs.add(Breadcrumb(category: "test", to: "point b", from: "point a"))
	...
	@IBAction func onClickMessage(_ sender: AnyObject) {
    ...
    @IBAction func onClickFatalError(_ sender: AnyObject) {
        SentryClient.shared?.crash()
    }

	@IBAction func onClickComplexMessage(_ sender: AnyObject) {
