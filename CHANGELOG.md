...
- Add package traits for UI framework opt-out and macOS CLI sample (#7578).
  ...
  In Xcode 26.2 or later, add the Sentry package as a dependency and add the `SentrySPM` product, then enable the `NoUIFramework` trait on the package reference (Package Dependencies → select Sentry → Traits).
