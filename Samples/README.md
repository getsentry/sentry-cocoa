...
Most subdirectories contain an xcodeproj per platform/version, with one or more targets describing variants of applications targeting it, and including the Sentry SDK via a local Swift package reference (`packages:` in the XcodeGen spec). Each sample xcodeproj is included in the top level Sentry.xcworkspace.
