...
class TraceTestViewController: UIViewController {
    ...
    func assertTransaction(span: Span) {
        DispatchQueue.main.async {
            ...
            guard children.contains(where: { $0.operation == "http.client" && $0.data["url"] as? String == "https://sentry-brand.storage.googleapis.com/sentry-logo-black.png" && $0.data["http.response.status_code"] as? NSNumber == NSNumber(value: 200) }) else {
