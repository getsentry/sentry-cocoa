    ...
    [SentrySDK startWithConfigureOptions:^(SentryOptions *options) {
        ...
        options.configureUserFeedback = ^(SentryUserFeedbackConfiguration * _Nonnull config) {
            ...
            config.configureWidget = ^(SentryUserFeedbackWidgetConfiguration *_Nonnull widget) {
                if ([args containsObject:@"--io.sentry.iOS-Swift.auto-inject-user-feedback-widget"]) {
            ...
            config.configureForm = ^(SentryUserFeedbackFormConfiguration *_Nonnull uiForm) {
                ...
                uiForm.lightThemeOverrides = ^(SentryUserFeedbackThemeConfiguration *_Nonnull lightTheme) {
            ...
            config.onSubmitSuccess = ^(NSDictionary<NSString *,id> *_Nonnull info) {
                ...
                UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Thanks?" message:[NSString stringWithFormat:@"We have enough jank of our own, we really didn't need yours too, %@", name] preferredStyle:UIAlertControllerStyleAlert];
                [alert addAction:[UIAlertAction actionWithTitle:@"Derp" style:UIAlertActionStyleDefault handler:nil]];
                [self.window.rootViewController presentViewController:alert animated:YES completion:nil];
            };
            config.onSubmitError = ^(NSError *_Nonnull error) {
                UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"D'oh" message:[NSString stringWithFormat:@"You tried to report jank, and encountered more jank. The jank has you now: %@", error] preferredStyle:UIAlertControllerStyleAlert];
                [alert addAction:[UIAlertAction actionWithTitle:@"Derp" style:UIAlertActionStyleDefault handler:nil]];
                [self.window.rootViewController presentViewController:alert animated:YES completion:nil];
            };
        };
