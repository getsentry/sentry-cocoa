...
protected:
    ...
    void
    grow_pod(size_t MinSize, size_t TSize)
    {
    ...
    bool
    isReferenceToRange(const void *V, const void *First, const void *Last) const
    {
    ...
    bool
    isReferenceToStorage(const void *V) const
    {
    ...
    /// vector's storage.
    bool
    isRangeInStorage(const void *First, const void *Last) const
    {
        // Use std::less to avoid UB.
        std::less<> LessThan;
    ...
    /// NewSize.
    bool
    isSafeToReferenceAfterResize(const void *Elt, size_t NewSize)
    {
        // Past the end.
    ...
    void
    assertSafeToReferenceAfterResize(const void *Elt, size_t NewSize)
    {
    ...
    /// vector by N.
    void
    assertSafeToAdd(const void *Elt, size_t N = 1)
    {
    ...
    void
    assertSafeToReferenceAfterClear(const T *From, const T *To)
    {
        if (From == To)
            return;
    ...
    template <class ItTy,
        std::enable_if_t<!std::is_same<std::remove_const_t<ItTy>, T *>::value, bool> = false>
    void
    assertSafeToReferenceAfterClear(ItTy, ItTy)
    {
    }

    /// Check whether any part of the range will be invalidated by growing.
    void
    assertSafeToAddRange(const T *From, const T *To)
    {
        if (From == To)
            return;
    ...
    template <class ItTy,
        std::enable_if_t<!std::is_same<std::remove_const_t<ItTy>, T *>::value, bool> = false>
    void
    assertSafeToAddRange(ItTy, ItTy)
    {
    }
...
template <typename T,
    bool = (is_trivially_copy_constructible<T>::value)
        && (is_trivially_move_constructible<T>::value) && std::is_trivially_destructible<T>::value>
...
protected:
    ...
    T *
    mallocForGrow(size_t MinSize, size_t &NewCapacity)
    {
    ...
    /// pointer in case it was a reference to the storage.
    const T *
    reserveForParamAndGetAddress(const T &Elt, size_t N = 1)
    {
    ...
    /// pointer in case it was a reference to the storage.
    T *
    reserveForParamAndGetAddress(T &Elt, size_t N = 1)
    {
        return const_cast<T *>(this->reserveForParamAndGetAddressImpl(this, Elt, N));
    }

    static T &&
    forward_value_param(T &&V)
    {
        return std::move(V);
    }
    static const T &
    forward_value_param(const T &V)
    {
        return V;
    }

    void
    growAndAssign(size_t NumElts, const T &Elt)
    {
    ...
    T &
    growAndEmplaceBack(ArgTypes &&...Args)
    {
...
template <typename T, bool TriviallyCopyable>
void
SmallVectorTemplateBase<T, TriviallyCopyable>::moveElementsForGrow(T *NewElts)
{
...
template <typename T, bool TriviallyCopyable>
void
SmallVectorTemplateBase<T, TriviallyCopyable>::takeAllocationForGrow(T *NewElts, size_t NewCapacity)
{
...
protected:
    ...
    static ValueParamT
    forward_value_param(ValueParamT V)
    {
        return V;
    }

    void
    growAndAssign(size_t NumElts, T Elt)
    {
...
public:
    ...
    void
    resize_for_overwrite(size_type N)
    {
        resizeImpl<true>(N);
    }
    ...
    void
    resize(size_type N, ValueParamT NV)
    {
        if (N == this->size())
            return;
    ...
    void
    append(size_type NumInputs, ValueParamT Elt)
    {
    ...
    void
    append(const SmallVectorImpl &RHS)
    {
        append(RHS.begin(), RHS.end());
    }

    void
    assign(size_type NumElts, ValueParamT Elt)
    {
    ...
    template <typename in_iter,
        typename = std::enable_if_t<
            std::is_convertible<typename std::iterator_traits<in_iter>::iterator_category,
                std::input_iterator_tag>::value>>
    void
    assign(in_iter in_start, in_iter in_end)
    {
    ...
    void
    assign(const SmallVectorImpl &RHS)
    {
...
private:
    ...
    iterator
    insert_one_impl(iterator I, ArgType &&Elt)
    {
...
public:
    ...
    iterator
    insert(iterator I, size_type NumToInsert, ValueParamT Elt)
    {
...
SmallVector<typename std::remove_const<typename std::remove_reference<decltype(*std::begin(
                std::declval<R &>()))>::type>::type,
    Size>
to_vector(R &&Range)
{
