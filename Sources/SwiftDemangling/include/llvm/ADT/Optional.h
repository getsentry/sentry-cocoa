...
namespace optional_detail {
    ...
    public:
        ...
        OptionalStorage &
        operator=(OptionalStorage const &other)
        {
            if (other.hasValue()) {
                if (hasValue()) {
                    value = other.value;
        ...
        OptionalStorage &
        operator=(OptionalStorage &&other)
        {
            if (other.hasValue()) {
                if (hasValue()) {
...
public:
    ...
    template <typename U>
    constexpr T
    getValueOr(U &&value) const LLVM_LVALUE_FUNCTION
    {
    ...
    auto
    map(const Function &F) const LLVM_LVALUE_FUNCTION->Optional<decltype(F(getValue()))>
    {
        if (*this)
            return F(getValue());
        return None;
    ...
    template <class Function>
    auto
    map(const Function &F) && -> Optional<decltype(F(std::move(*this).getValue()))>
    {
        if (*this)
...
template <typename T>
constexpr bool
operator==(const Optional<T> &X, NoneType)
{
    return !X;
}
...
template <typename T>
constexpr bool
operator==(NoneType, const Optional<T> &X)
{
    return X == None;
}
...
template <typename T>
constexpr bool
operator<(const Optional<T> &, NoneType)
{
    return false;
}
...
template <typename T,
    typename = decltype(std::declval<raw_ostream &>() << std::declval<const T &>())>
raw_ostream &
operator<<(raw_ostream &OS, const Optional<T> &O)
{
    if (O)
        OS << *O;
    else
