...
namespace detail {
    ...
    template <typename RangeT> using IterOfRange = decltype(std::begin(std::declval<RangeT &>()));

    template <typename RangeT>
    using ValueOfRange =
        typename std::remove_reference<decltype(*std::begin(std::declval<RangeT &>()))>::type;
...
struct function_traits<ReturnType (*)(Args...), false> {
    ...
    /// The type of an argument to this function.
    template <size_t i> using arg_t = typename std::tuple_element<i, std::tuple<Args...>>::type;
};
...
template <class Ty> struct identity {
    ...
    Ty &
    operator()(Ty &self) const
    {
        return self;
    }
    const Ty &
    operator()(const Ty &self) const
    {
        return self;
    }
};
...
public:
    ...
    function_ref(Callable &&callable,
        ...
        std::enable_if_t<!std::is_same<remove_cvref_t<Callable>, function_ref>::value> * = nullptr,
...
namespace adl_detail {
    ...
    decltype(auto)
    adl_begin(ContainerTy &&container)
    {
    ...
    template <typename T>
    void
    adl_swap(T &&lhs, T &&rhs) noexcept(noexcept(swap(std::declval<T>(), std::declval<T>())))
    {
...
template <typename T>
constexpr bool
empty(const T &RangeOrContainer)
{
...
template <typename ContainerTy>
bool
hasSingleElement(ContainerTy &&C)
{
...
template <typename T>
auto
drop_begin(T &&RangeOrContainer, size_t N = 1)
{
    return make_range(std::next(adl_begin(RangeOrContainer), N), adl_end(RangeOrContainer));
}
...
class mapped_iterator : public iterator_adaptor_base<mapped_iterator<ItTy, FuncTy>, ItTy,
...
inline mapped_iterator<ItTy, FuncTy>
map_iterator(ItTy I, FuncTy F)
{
...
auto
map_range(ContainerTy &&C, FuncTy F)
{
...
template <typename ContainerTy>
auto
reverse(ContainerTy &&C, std::enable_if_t<has_rbegin<ContainerTy>::value> * = nullptr)
{
...
std::reverse_iterator<IteratorTy>
...
{
    return make_range(
        llvm::make_reverse_iterator(std::end(C)), llvm::make_reverse_iterator(std::begin(C)));
}
...
class filter_iterator_base
    : public iterator_adaptor_base<filter_iterator_base<WrappedIteratorT, PredicateT, IterTag>,
          WrappedIteratorT,
          typename std::common_type<IterTag,
              typename std::iterator_traits<WrappedIteratorT>::iterator_category>::type> {
    using BaseT = iterator_adaptor_base<filter_iterator_base<WrappedIteratorT, PredicateT, IterTag>,
        WrappedIteratorT,
        typename std::common_type<IterTag,
            typename std::iterator_traits<WrappedIteratorT>::iterator_category>::type>;
...
namespace detail {
    ...
    template <typename IterT> struct fwd_or_bidi_tag {
        using type = typename fwd_or_bidi_tag_impl<std::is_base_of<std::bidirectional_iterator_tag,
...
template <typename WrappedIteratorT, typename PredicateT>
using filter_iterator = filter_iterator_impl<WrappedIteratorT, PredicateT,
...
make_filter_range(RangeT &&Range, PredicateT Pred)
{
    ...
    return make_range(FilterIteratorT(std::begin(std::forward<RangeT>(Range)),
        ...
        FilterIteratorT(
            std::end(std::forward<RangeT>(Range)), std::end(std::forward<RangeT>(Range)), Pred));
}
...
class early_inc_iterator_impl
    : public iterator_adaptor_base<early_inc_iterator_impl<WrappedIteratorT>, WrappedIteratorT,
          std::input_iterator_tag> {
    using BaseT = iterator_adaptor_base<early_inc_iterator_impl<WrappedIteratorT>, WrappedIteratorT,
        std::input_iterator_tag>;
...
namespace detail {
    ...
    using zip_traits = iterator_facade_base<ZipType,
    ...
    protected:
        ...
        template <size_t... Ns>
        decltype(iterators)
        tup_inc(std::index_sequence<Ns...>) const
        {
        ...
        template <size_t... Ns>
        decltype(iterators)
        tup_dec(std::index_sequence<Ns...>) const
        {
    ...
    public:
        zip_common(Iters &&...ts)
            : iterators(std::forward<Iters>(ts)...)
        {
        }
    ...
    struct zip_first : public zip_common<zip_first<Iters...>, Iters...> {
        ...
        bool
        operator==(const zip_first<Iters...> &other) const
        {
    ...
    class zip_shortest : public zip_common<zip_shortest<Iters...>, Iters...> {
        template <size_t... Ns>
        bool
        test(const zip_shortest<Iters...> &other, std::index_sequence<Ns...>) const
        {
            return all_of(std::initializer_list<bool> { std::get<Ns>(this->iterators)
                              != std::get<Ns>(other.iterators)... },
                identity<bool> {});
    ...
    public:
        ...
        bool
        operator==(const zip_shortest<Iters...> &other) const
        {
...
detail::zippy<detail::zip_shortest, T, U, Args...>
zip(T &&t, U &&u, Args &&...args)
{
...
/// be the shortest.
template <typename T, typename U, typename... Args>
detail::zippy<detail::zip_first, T, U, Args...>
...
namespace detail {
    ...
    Iter
    next_or_end(const Iter &I, const Iter &End)
    {
        if (I == End)
            return End;
        return std::next(I);
    ...
    template <typename Iter> struct ZipLongestItemType {
        using type = llvm::Optional<typename std::remove_const<
            typename std::remove_reference<decltype(*std::declval<Iter>())>::type>::type>;
    };
    ...
    class zip_longest_iterator
        : public iterator_facade_base<zip_longest_iterator<Iters...>,
    ...
    private:
        ...
        template <size_t... Ns>
        bool
        test(const zip_longest_iterator<Iters...> &other, std::index_sequence<Ns...>) const
        {
            return llvm::any_of(std::initializer_list<bool> { std::get<Ns>(this->iterators)
                                    != std::get<Ns>(other.iterators)... },
                identity<bool> {});
        }
    ...
    public:
        ...
        bool
        operator==(const zip_longest_iterator<Iters...> &other) const
        {
            return !test(other, std::index_sequence_for<Iters...> {});
        }
    };
...
class concat_iterator : public iterator_facade_base<concat_iterator<ValueT, IterTs...>,
    ...
    template <size_t... Ns>
    void
    increment(std::index_sequence<Ns...>)
    {
    ...
    template <size_t... Ns>
    ValueT &
    get(std::index_sequence<Ns...>) const
    {
...
public:
    ...
    bool
    operator==(const concat_iterator &RHS) const
    {
...
namespace detail {
    ...
    public:
        using iterator
            = concat_iterator<ValueT, decltype(std::begin(std::declval<RangeTs &>()))...>;
    ...
    private:
        ...
        template <size_t... Ns>
        iterator
        end_impl(std::index_sequence<Ns...>)
        {
            return iterator(
                make_range(std::end(std::get<Ns>(Ranges)), std::end(std::get<Ns>(Ranges)))...);
        }
...
detail::concat_range<ValueT, RangeTs...>
...
class indexed_accessor_iterator
    : public llvm::iterator_facade_base<DerivedT, std::random_access_iterator_tag, T,
          std::ptrdiff_t, PointerT, ReferenceT> {
public:
    ptrdiff_t
    operator-(const indexed_accessor_iterator &rhs) const
    {
...
namespace detail {
    ...
    public:
        ...
        class iterator
            : public indexed_accessor_iterator<iterator, BaseT, T, PointerT, ReferenceT> {
        public:
        ...
        {
            return size() == static_cast<size_t>(std::distance(other.begin(), other.end()))
    ...
    private:
        ...
        static BaseT
        offset_base(const BaseT &base, size_t n)
        {
...
template <typename DerivedT, typename BaseT, typename T, typename PointerT = T *,
    typename ReferenceT = T &>
class indexed_accessor_range : public detail::indexed_accessor_range_base<DerivedT,
                                   std::pair<BaseT, ptrdiff_t>, T, PointerT, ReferenceT> {
public:
    indexed_accessor_range(BaseT base, ptrdiff_t startIndex, ptrdiff_t count)
        : detail::indexed_accessor_range_base<DerivedT, std::pair<BaseT, ptrdiff_t>, T, PointerT,
            ReferenceT>(std::make_pair(base, startIndex), count)
    {
    }
    ...
    /// See `detail::indexed_accessor_range_base` for details.
    static ReferenceT
    dereference_iterator(const std::pair<BaseT, ptrdiff_t> &base, ptrdiff_t index)
    {
...
make_first_range(ContainerTy &&c)
{
    return llvm::map_range(std::forward<ContainerTy>(c),
        [](decltype((*std::begin(c))) elt) -> decltype((elt.first)) { return elt.first; });
}
...
{
    return llvm::map_range(std::forward<ContainerTy>(c),
        [](decltype((*std::begin(c))) elt) -> decltype((elt.second)) { return elt.second; });
}
...
template <typename T, typename... Ts> using are_base_of = conjunction<std::is_base_of<T, Ts>...>;
...
makeVisitor(CallableTs &&...Callables)
{
...
void
shuffle(Iterator first, Iterator last, RNG &&g)
{
...
template <typename T>
inline int
array_pod_sort_comparator(const void *P1, const void *P2)
{
...
template <typename T>
inline int (*get_array_pod_sort_comparator(const T &))(const void *, const void *)
{
...
namespace detail {
    ...
    inline void
    presortShuffle(IteratorTy Start, IteratorTy End)
    {
...
template <class IteratorTy>
inline void
array_pod_sort(IteratorTy Start, IteratorTy End)
{
...
namespace detail {
    ...
    using sort_trivially_copyable = conjunction<std::is_pointer<T>,
...
template <typename IteratorTy,
    std::enable_if_t<!detail::sort_trivially_copyable<IteratorTy>::value, int> = 0>
inline void
...
UnaryFunction
for_each(R &&Range, UnaryFunction F)
{
...
bool
all_of(R &&Range, UnaryPredicate P)
{
...
template <typename R, typename UnaryPredicate>
bool
any_of(R &&Range, UnaryPredicate P)
{
...
template <typename R, typename UnaryPredicate>
bool
none_of(R &&Range, UnaryPredicate P)
{
...
template <typename R, typename UnaryPredicate>
auto
find_if(R &&Range, UnaryPredicate P)
{
...
template <typename R, typename UnaryPredicate>
auto
find_if_not(R &&Range, UnaryPredicate P)
{
...
template <typename R, typename UnaryPredicate>
auto
remove_if(R &&Range, UnaryPredicate P)
{
...
bool
is_contained(R &&Range, const E &Element)
{
...
template <typename R, typename UnaryPredicate>
auto
count_if(R &&Range, UnaryPredicate P)
{
...
OutputIt
transform(R &&Range, OutputIt d_first, UnaryFunction F)
{
...
template <typename R, typename UnaryPredicate>
auto
partition(R &&Range, UnaryPredicate P)
{
...
auto
lower_bound(R &&Range, T &&Value, Compare C)
{
    return std::lower_bound(adl_begin(Range), adl_end(Range), std::forward<T>(Value), C);
}
...
template <typename R, typename T, typename Compare>
auto
upper_bound(R &&Range, T &&Value, Compare C)
{
    return std::upper_bound(adl_begin(Range), adl_end(Range), std::forward<T>(Value), C);
}
...
auto
partition_point(R &&Range, Predicate P)
{
...
bool
equal(L &&LRange, R &&RRange)
{
    return std::equal(adl_begin(LRange), adl_end(LRange), adl_begin(RRange), adl_end(RRange));
}
...
is_splat(R &&Range)
{
    ...
    return range_size != 0
        && (range_size == 1 || std::equal(adl_begin(Range) + 1, adl_end(Range), adl_begin(Range)));
}
...
void
erase_if(Container &C, UnaryPredicate P)
{
...
void
erase_value(Container &C, ValueType V)
{
...
inline void
append_range(Container &C, Range &&R)
{
    C.insert(C.end(), R.begin(), R.end());
}
...
void
replace(Container &Cont, typename Container::iterator ContIt, typename Container::iterator ContEnd,
...
/// the range R.
template <typename Container,
    typename Range = std::initializer_list<typename Container::value_type>>
void
replace(Container &Cont, typename Container::iterator ContIt, typename Container::iterator ContEnd,
    Range R)
{
...
template <typename ForwardIterator, typename UnaryFunctor, typename NullaryFunctor,
    typename = typename std::enable_if<!std::is_constructible<StringRef, UnaryFunctor>::value
        && !std::is_constructible<StringRef, NullaryFunctor>::value>::type>
inline void
interleave(
    ForwardIterator begin, ForwardIterator end, UnaryFunctor each_fn, NullaryFunctor between_fn)
{
    if (begin == end)
        return;
    each_fn(*begin);
...
template <typename Container, typename UnaryFunctor, typename NullaryFunctor,
    typename = typename std::enable_if<!std::is_constructible<StringRef, UnaryFunctor>::value
        && !std::is_constructible<StringRef, NullaryFunctor>::value>::type>
inline void
interleave(const Container &c, UnaryFunctor each_fn, NullaryFunctor between_fn)
{
...
inline void
interleave(const Container &c, StreamT &os, UnaryFunctor each_fn, const StringRef &separator)
{
    interleave(c.begin(), c.end(), each_fn, [&] { os << separator; });
}
template <typename Container, typename StreamT, typename T = detail::ValueOfRange<Container>>
inline void
interleave(const Container &c, StreamT &os, const StringRef &separator)
{
    interleave(
...
template <typename Container, typename UnaryFunctor, typename StreamT,
    typename T = detail::ValueOfRange<Container>>
inline void
interleaveComma(const Container &c, StreamT &os, UnaryFunctor each_fn)
{
    interleave(c, os, each_fn, ", ");
}
template <typename Container, typename StreamT, typename T = detail::ValueOfRange<Container>>
inline void
interleaveComma(const Container &c, StreamT &os)
{
...
template <typename First, typename Second> struct pair_hash {
    size_t
    operator()(const std::pair<First, Second> &P) const
    {
...
namespace detail {
    ...
    template <typename R> struct result_pair {
        ...
        result_pair &
        operator=(const result_pair &Other)
        {
            Index = Other.Index;
            Iter = Other.Iter;
            return *this;
    ...
    class enumerator_iter
        : public iterator_facade_base<enumerator_iter<R>, std::forward_iterator_tag, result_pair<R>,
    ...
    public:
        ...
        bool
        operator==(const enumerator_iter &RHS) const
        {
        ...
        enumerator_iter &
        operator=(const enumerator_iter &Other)
        {
            Result = Other.Result;
            return *this;
        }
...
namespace detail {
    ...
    decltype(auto)
    apply_tuple_impl(F &&f, Tuple &&t, std::index_sequence<I...>)
    {
...
namespace detail {
    ...
    bool
    all_of_zip_predicate_first(Predicate &&P, Args &&...args)
    {
        auto z = zip(args...);
        auto it = z.begin();
        auto end = z.end();
    ...
    all_of_zip_predicate_last(
        std::tuple<ArgsThenPredicate...> argsThenPredicate, std::index_sequence<InputIndexes...>)
    {
        auto constexpr OutputIndex = std::tuple_size<decltype(argsThenPredicate)>::value - 1;
        return all_of_zip_predicate_first(
            std::get<OutputIndex>(argsThenPredicate), std::get<InputIndexes>(argsThenPredicate)...);
    }
...
bool
all_of_zip(ArgsAndPredicate &&...argsAndPredicate)
{
...
template <typename ContainerTy>
bool
hasNItems(ContainerTy &&C, unsigned N)
{
...
template <typename ContainerTy>
bool
hasNItemsOrMore(ContainerTy &&C, unsigned N)
{
...
template <typename ContainerTy>
bool
hasNItemsOrLess(ContainerTy &&C, unsigned N)
{
