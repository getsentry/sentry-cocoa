...
private:
    ...
    enum class BufferKind { Unbuffered = 0, InternalBuffer, ExternalBuffer } BufferMode;
...
public:
    ...
    explicit raw_ostream(bool unbuffered = false, OStreamKind K = OStreamKind::OK_OStream)
        : Kind(K)
        , BufferMode(unbuffered ? BufferKind::Unbuffered : BufferKind::InternalBuffer)
    {
...
std::enable_if_t<!std::is_reference<OStream>::value && std::is_base_of<raw_ostream, OStream>::value,
    OStream &&>
...
public:
    explicit raw_pwrite_stream(bool Unbuffered = false, OStreamKind K = OStreamKind::OK_OStream)
...
protected:
    ...
    void
    error_detected(std::error_code EC)
    {
        this->EC = EC;
    }
...
public:
    ...
    raw_fd_ostream(
        int fd, bool shouldClose, bool unbuffered = false, OStreamKind K = OStreamKind::OK_OStream);
...
public:
    ...
    void
    reserveExtraSpace(uint64_t ExtraSize) override
    {
