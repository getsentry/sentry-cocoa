...
#define SWIFT_INLINE_BITFIELD(T, U, C, ...) SWIFT_INLINE_BITFIELD_TEMPLATE(T, U, C, 0, __VA_ARGS__)
...
#define SWIFT_INLINE_BITFIELD_EMPTY(T, U) enum { Num##T##Bits = Num##U##Bits }

// XXX/HACK: templated max() doesn't seem to work in a bitfield size context.
constexpr unsigned
bitmax(unsigned a, unsigned b)
{
    return a > b ? a : b;
}

constexpr unsigned
countBitsUsed(uint64_t arg)
{
