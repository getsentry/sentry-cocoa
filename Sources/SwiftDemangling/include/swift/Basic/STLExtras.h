...
template <class T, class R, class... Args> struct function_traits<R (T::*)(Args...)> {
    using result_type = R;
    using argument_types = std::tuple<Args...>;
};

template <class T, class R, class... Args> struct function_traits<R (T::*)(Args...) const> {
    using result_type = R;
    using argument_types = std::tuple<Args...>;
};
...
inline void
for_each(InputIt1 I1, InputIt1 E1, InputIt2 I2, BinaryFunction f)
{
    while (I1 != E1) {
        f(*I1, *I2);
        ++I1;
...
inline void
for_each(const Container1 &c1, const Container2 &c2, BinaryFunction f)
{
...
template <typename InputIt1, typename InputIt2, typename InputIt3, typename TernaryFunction>
inline void
for_each3(InputIt1 I1, InputIt1 E1, InputIt2 I2, InputIt3 I3, TernaryFunction f)
{
    while (I1 != E1) {
        f(*I1, *I2, *I3);
        ++I1;
...
template <typename Container1, typename Container2, typename Container3, typename TernaryFunction>
inline void
for_each3(const Container1 &c1, const Container2 &c2, const Container3 &c3, TernaryFunction f)
{
    assert(c1.size() == c2.size());
...
template <typename InputIt1, typename InputIt2, typename BinaryFunction>
inline void
set_union_for_each(InputIt1 I1, InputIt1 E1, InputIt2 I2, InputIt2 E2, BinaryFunction f)
{
    static_assert(std::is_same<typename std::iterator_traits<InputIt1>::value_type,
...
inline void
set_union_for_each(const Container1 &C1, const Container2 &C2, UnaryFunction f)
{
    // Make sure that our iterators have the same value type.
    static_assert(
        std::is_same<typename std::iterator_traits<typename Container1::iterator>::value_type,
            typename std::iterator_traits<typename Container2::iterator>::value_type>::value,
...
inline Iterator
next_or_default(Iterator it, Iterator end, Iterator defaultIter)
{
...
template <typename Iterator>
inline Iterator
prev_or_default(Iterator it, Iterator begin, Iterator defaultIter)
{
...
template <typename Iterator>
inline Iterator
next_or_end(Iterator it, Iterator end)
{
...
template <typename Iterator>
inline Iterator
prev_or_begin(Iterator it, Iterator begin)
{
...
/// a null pointer.
template <class T, T *(&getNext)(T *)> class LinkedListIterator {
    T *Pointer;
...
public:
    ...
    static llvm::iterator_range<LinkedListIterator>
    rangeBeginning(T *pointer)
    {
        return { pointer, nullptr };
    }

    constexpr LinkedListIterator(T *pointer)
        : Pointer(pointer)
    {
    }
    ...
    friend bool
    operator==(LinkedListIterator lhs, LinkedListIterator rhs)
    {
...
template <typename Iterator, typename Operation> class TransformIterator {
    Iterator Current;
...
public:
    ...
    friend bool
    operator==(TransformIterator lhs, TransformIterator rhs)
    {
...
template <typename Range, typename Operation> class TransformRange {
    Range Rng;
    Operation Op;
...
public:
    ...
    begin() const
    {
        return iterator(Rng.begin(), Op);
    }
    iterator
    end() const
    {
        return iterator(Rng.end(), Op);
    }
    bool
    empty() const
    {
...
/// be skipped.
template <typename Iterator, typename OptionalTransform> class OptionalTransformIterator {
    ...
    using UnderlyingReference = typename std::iterator_traits<Iterator>::reference;

    using ResultReference = typename std::result_of<OptionalTransform(UnderlyingReference)>::type;
...
public:
    ...
    OptionalTransformIterator(Iterator current, Iterator end, OptionalTransform op)
        : Current(current)
        , End(end)
        , Op(op)
    {
    ...
    OptionalTransformIterator(Iterator current, Iterator end, OptionalTransform op, PrimedT)
        : Current(current)
        , End(end)
        , Op(op)
    {
    ...
    friend bool
    operator==(OptionalTransformIterator lhs, OptionalTransformIterator rhs)
    {
        return lhs.Current == rhs.Current;
    }
    friend bool
...
makeOptionalTransformIterator(Iterator current, Iterator end, OptionalTransform op)
{
    return OptionalTransformIterator<Iterator, OptionalTransform>(current, end, op);
}
...
template <typename Subclass> struct DowncastAsOptional {
    template <typename Superclass>
    auto
    operator()(Superclass &value) const -> llvm::Optional<decltype(llvm::cast<Subclass>(value))>
    {
    ...
    template <typename Superclass>
    auto
    operator()(const Superclass &value) const
...
using DowncastFilterIterator = OptionalTransformIterator<Iterator, DowncastAsOptional<Subclass>>;
...
class DowncastFilterRange : public OptionalTransformRange<Range, DowncastAsOptional<Subclass>> {
...
void
sortUnique(Container &C, Comparator Cmp,
    typename std::enable_if<
        std::is_same<typename std::iterator_traits<typename Container::iterator>::iterator_category,
            std::random_access_iterator_tag>::value,
        void>::type * = nullptr)
...
inline bool
is_sorted_and_uniqued(IterTy II, IterTy IE)
{
...
template <typename Container>
inline bool
is_sorted_and_uniqued(const Container &C)
{
...
inline T
accumulate(const Container &C, T init, BinaryOperation op)
{
...
template <typename Container, typename T, typename BinaryOperation>
inline bool
binary_search(const Container &C, const T &value, BinaryOperation op)
{
...
inline bool
hasPrefix(MainInputIterator mainBegin, const MainInputIterator mainEnd,
    PrefixInputIterator prefixBegin, const PrefixInputIterator prefixEnd)
{
...
template <typename T> class RelationalOperationsBase {
public:
    friend bool
...
Iterator
removeAdjacentIf(const Iterator first, const Iterator last, BinaryPredicate pred)
{
    using element_reference_t = typename std::iterator_traits<Iterator>::reference;
    ...
    while (nextOverlap != last) {
        ...
        auto lastOverlapInRun = std::adjacent_find(std::next(nextOverlap), last,
            [&pred](element_reference_t left, element_reference_t right) -> bool {
...
template <bool... b>
using all_true = std::is_same<detail::bool_pack<b..., true>, detail::bool_pack<true, b...>>;

/// traits class for checking whether Ts consists only of compound types.
template <class... Ts> using are_all_compound = all_true<std::is_compound<Ts>::value...>;
