...
#if (__has_attribute(visibility) || SWIFT_GNUC_PREREQ(4, 0, 0)) && !defined(__MINGW32__)           \
...
#        define SWIFT_CLASS_IS_SWIFT_MASK                                                          \
            __COMPATIBILITY_LIBRARIES_CANNOT_CHECK_THE_IS_SWIFT_BIT_DIRECTLY__
...
#    define SWIFT_ALLOWED_RUNTIME_GLOBAL_CTOR_END _Pragma("clang diagnostic pop")
#else
#    define SWIFT_ALLOWED_RUNTIME_GLOBAL_CTOR_BEGIN
...
#if __has_attribute(preserve_most) && (defined(__aarch64__) || defined(__x86_64__))
...
#    define __ptrauth_swift_job_invoke_function                                                    \
        __ptrauth(                                                                                 \
            ptrauth_key_function_pointer, 1, SpecialPointerAuthDiscriminators::JobInvokeFunction)
#    define __ptrauth_swift_task_resume_function                                                   \
        __ptrauth(                                                                                 \
            ptrauth_key_function_pointer, 1, SpecialPointerAuthDiscriminators::TaskResumeFunction)
...
#    define __ptrauth_swift_async_context_resume                                                   \
        __ptrauth(                                                                                 \
            ptrauth_key_function_pointer, 1, SpecialPointerAuthDiscriminators::AsyncContextResume)
#    define __ptrauth_swift_async_context_yield                                                    \
        __ptrauth(                                                                                 \
            ptrauth_key_function_pointer, 1, SpecialPointerAuthDiscriminators::AsyncContextYield)
...
#    define swift_ptrauth_sign_opaque_modify_resume_function(__fn, __buffer)                       \
        ptrauth_auth_and_resign(__fn, ptrauth_key_function_pointer, 0,                             \
            ptrauth_key_process_independent_code,                                                  \
            ptrauth_blend_discriminator(                                                           \
                __buffer, SpecialPointerAuthDiscriminators::OpaqueModifyResumeFunction))
#else
...
swift_ptrauth_copy(T *dest, const T *src, unsigned extra)
{
#    if SWIFT_PTRAUTH
    *dest = ptrauth_auth_and_resign(*src, ptrauth_key_function_pointer,
        ptrauth_blend_discriminator(src, extra), ptrauth_key_function_pointer,
...
template <class T>
SWIFT_RUNTIME_ATTRIBUTE_ALWAYS_INLINE static inline void
swift_ptrauth_copy_data(T *dest, const T *src, unsigned extra)
{
#    if SWIFT_PTRAUTH
    *dest = ptrauth_auth_and_resign(*src, ptrauth_key_process_independent_data,
        ptrauth_blend_discriminator(src, extra), ptrauth_key_process_independent_data,
        ptrauth_blend_discriminator(dest, extra));
#    else
    *dest = *src;
#    endif
...
template <class T>
SWIFT_RUNTIME_ATTRIBUTE_ALWAYS_INLINE static inline void
swift_ptrauth_copy_code_or_data(T *dest, const T *src, unsigned extra, bool isCode)
{
    if (isCode) {
...
template <class T>
SWIFT_RUNTIME_ATTRIBUTE_ALWAYS_INLINE static inline void
swift_ptrauth_init(T *dest, T value, unsigned extra)
{
    // FIXME: assert that T is not a function-pointer type?
#    if SWIFT_PTRAUTH
    *dest = ptrauth_sign_unauthenticated(
        value, ptrauth_key_function_pointer, ptrauth_blend_discriminator(dest, extra));
#    else
    *dest = value;
#    endif
}
...
template <class T>
SWIFT_RUNTIME_ATTRIBUTE_ALWAYS_INLINE static inline void
swift_ptrauth_init_data(T *dest, T value, unsigned extra)
{
    // FIXME: assert that T is not a function-pointer type?
#    if SWIFT_PTRAUTH
    *dest = ptrauth_sign_unauthenticated(
        value, ptrauth_key_process_independent_data, ptrauth_blend_discriminator(dest, extra));
#    else
    *dest = value;
#    endif
}
...
swift_auth_data_non_address(T value, unsigned extra)
{
#    if SWIFT_PTRAUTH
    return (T)ptrauth_auth_data((void *)value, ptrauth_key_process_independent_data, extra);
#    else
    return value;
#    endif
}
...
{
#    if SWIFT_PTRAUTH
    return (T)ptrauth_auth_function((void *)value, ptrauth_key_process_independent_code, extra);
#    else
    return value;
#    endif
}
