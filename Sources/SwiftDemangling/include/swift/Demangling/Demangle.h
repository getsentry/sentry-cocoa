...
namespace Demangle {
    ...
    struct DemangleOptions {
        ...
        std::function<std::string(uint64_t, uint64_t)> GenericParameterName = genericParameterName;

        DemangleOptions() { }

        static DemangleOptions
    ...
    static inline char
    encodeSpecializationPass(SpecializationPass Pass)
    {
...
#include "swift/Demangling/DemangleNodes.def"
    ...
    private:
        ...
        enum class PayloadKind : uint8_t { None, Text, Index, OneChild, TwoChildren, ManyChildren };
        ...
        Node(Kind k, llvm::StringRef t)
            : NodeKind(k)
            , NodePayloadKind(PayloadKind::Text)
        {
            Text = t;
        }
        Node(Kind k, IndexType index)
            : NodeKind(k)
            , NodePayloadKind(PayloadKind::Index)
        {
            Index = index;
        }
    ...
    inline bool
    isMangledName(llvm::StringRef mangledName)
    {
    ...
    public:
        ...
        std::string demangleSymbolAsString(
            llvm::StringRef MangledName, const DemangleOptions &Options = DemangleOptions());
    ...
    /// \returns The demangled string.
    std::string demangleSymbolAsString(const char *mangledName, size_t mangledNameLength,
    ...
    demangleSymbolAsString(
        const std::string &mangledName, const DemangleOptions &options = DemangleOptions())
    {
    ...
    /// \param mangledNameLength The length of the mangledName string.
    /// \returns The demangled string.
    std::string demangleTypeAsString(const char *mangledName, size_t mangledNameLength,
        const DemangleOptions &options = DemangleOptions());
    ...
    using SymbolicResolver
        = llvm::function_ref<Demangle::NodePointer(SymbolicReferenceKind, const void *)>;
    ...
    llvm::StringRef mangleNode(NodePointer root, SymbolicResolver resolver, NodeFactory &Factory);
...
char *swift_demangle(const char *mangledName, size_t mangledNameLength, char *outputBuffer,
