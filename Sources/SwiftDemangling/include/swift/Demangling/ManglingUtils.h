...
namespace Mangle {
    ...
    inline bool
    isLowerLetter(char ch)
    {
        return ch >= 'a' && ch <= 'z';
    }

    inline bool
    isUpperLetter(char ch)
    {
        return ch >= 'A' && ch <= 'Z';
    }
    ...
    void
    mangleIdentifier(Mangler &M, StringRef ident)
    {
        ...
        for (size_t Pos = 0, Len = ident.size(); Pos <= Len; ++Pos) {
            ...
            if (wordStartPos != NotInsideWord && isWordEnd(ch, ident[Pos - 1])) {
                ...
                auto lookupWord = [&](StringRef Str, size_t FromWordIdx, size_t ToWordIdx) -> int {
        ...
        for (size_t Idx = 0, End = M.SubstWordsInIdent.size(); Idx < End; ++Idx) {
            ...
            if (Pos < Repl.StringPos) {
                ...
                do {
                    ...
                    if (WordsInBuffer < M.Words.size() && M.Words[WordsInBuffer].start == Pos) {
    ...
    public:
        ...
        template <typename Mangler>
        bool
        tryMergeSubst(Mangler &M, StringRef Subst, bool isStandardSubst)
        {
            assert(isUpperLetter(Subst.back()) || (isStandardSubst && isLowerLetter(Subst.back())));
            ...
            if (lastNumSubsts > 0 && lastNumSubsts < MaxRepeatCount
                ...
                StringRef lastSubst = BufferStr.take_back(lastSubstSize).drop_while([](char c) {
                    return isDigit(c);
                });
