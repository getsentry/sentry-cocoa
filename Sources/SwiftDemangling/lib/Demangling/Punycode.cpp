...
static bool
isValidUnicodeScalar(uint32_t S)
{
...
bool
Punycode::decodePunycode(StringRef InputPunycode, std::vector<uint32_t> &OutCodePoints)
{
    OutCodePoints.clear();
...
bool
Punycode::encodePunycode(const std::vector<uint32_t> &InputCodePoints, std::string &OutPunycode)
{
    OutPunycode.clear();
...
static bool
encodeToUTF8(const std::vector<uint32_t> &Scalars, std::string &OutUTF8)
{
    for (auto S : Scalars) {
...
bool
Punycode::decodePunycodeUTF8(StringRef InputPunycode, std::string &OutUTF8)
{
...
static bool
isContinuationByte(uint8_t unit)
{
...
static bool
convertUTF8toUTF32(
    llvm::StringRef InputUTF8, std::vector<uint32_t> &OutUTF32, bool mapNonSymbolChars)
{
    ...
    while (ptr < end) {
        ...
        if (first < 0x80) {
            ...
            if (!isContinuationByte(second) || !isContinuationByte(third))
                return false;
            code_point = ((first & 0xF) << 12) | ((second & 0x3F) << 6) | (third & 0x3F);
        } else if (first < 0xF8) {
            // Four-byte sequence.
            if (end - ptr < 3)
                return false;
            ...
            if (!isContinuationByte(second) || !isContinuationByte(third)
                ...
                return false;
            code_point = ((first & 0x7) << 18) | ((second & 0x3F) << 12) | ((third & 0x3F) << 6)
                | (fourth & 0x3F);
        } else {
...
bool
Punycode::encodePunycodeUTF8(StringRef InputUTF8, std::string &OutPunycode, bool mapNonSymbolChars)
{
