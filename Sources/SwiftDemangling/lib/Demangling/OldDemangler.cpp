...
static Node::Kind
nominalTypeMarkerToNodeKind(char c)
{
    if (c == 'C')
...
public:
    ...
    bool
    readUntil(char c, std::string &result)
    {
...
#define DEMANGLE_CHILD_AS_NODE_OR_RETURN(PARENT, CHILD_KIND)                                       \
    do {                                                                                           \
        ...
        if (!_kind.hasValue())                                                                     \
            return nullptr;                                                                        \
        addChild(PARENT, Factory.createNode(Node::Kind::CHILD_KIND, unsigned(*_kind)));            \
    } while (false)
    ...
    demangleTopLevel()
    {
        ...
        if (Mangled.nextIf("TS")) {
            ...
            addChild(topLevel, Factory.createNode(Node::Kind::DirectMethodReferenceAttribute));
        ...
        if (!Mangled.isEmpty()) {
            addChild(topLevel, Factory.createNode(Node::Kind::Suffix, Mangled.getString()));
        }
...
private:
    ...
    void
    addChild(NodePointer Parent, NodePointer Child)
    {
        Parent->addChild(Child, Factory);
    }

    llvm::Optional<Directness>
    demangleDirectness()
    {
        if (Mangled.nextIf('d'))
    ...
    bool
    demangleNatural(Node::IndexType &num)
    {
        if (!Mangled)
            return false;
        char c = Mangled.next();
    ...
    bool
    demangleBuiltinSize(Node::IndexType &num)
    {
    ...
    llvm::Optional<ValueWitnessKind>
    demangleValueWitnessKind()
    {
        char Code[2];
        if (!Mangled)
            return llvm::None;
...
#include "swift/Demangling/ValueWitnessMangling.def"
    ...
    demangleGlobal()
    {
        ...
        if (Mangled.nextIf('M')) {
            if (Mangled.nextIf('P')) {
                auto pattern = Factory.createNode(Node::Kind::GenericTypeMetadataPattern);
                DEMANGLE_CHILD_OR_RETURN(pattern, Type);
                return pattern;
            }
            if (Mangled.nextIf('a')) {
                auto accessor = Factory.createNode(Node::Kind::TypeMetadataAccessFunction);
            ...
            if (Mangled.nextIf('n')) {
                auto nominalType = Factory.createNode(Node::Kind::NominalTypeDescriptor);
        ...
        if (Mangled.nextIf('w')) {
            ...
            if (!w.hasValue())
                return nullptr;
            auto witness = Factory.createNode(Node::Kind::ValueWitness);
            NodePointer Idx = Factory.createNode(Node::Kind::Index, unsigned(w.getValue()));
        ...
        if (Mangled.nextIf('W')) {
            ...
            if (Mangled.nextIf('P')) {
                auto witnessTable = Factory.createNode(Node::Kind::ProtocolWitnessTable);
                DEMANGLE_CHILD_OR_RETURN(witnessTable, ProtocolConformance);
                return witnessTable;
            }
            if (Mangled.nextIf('G')) {
                auto witnessTable = Factory.createNode(Node::Kind::GenericProtocolWitnessTable);
                DEMANGLE_CHILD_OR_RETURN(witnessTable, ProtocolConformance);
                return witnessTable;
            }
            if (Mangled.nextIf('I')) {
            ...
            if (Mangled.nextIf('l')) {
                auto accessor = Factory.createNode(Node::Kind::LazyProtocolWitnessTableAccessor);
                DEMANGLE_CHILD_OR_RETURN(accessor, Type);
            ...
            if (Mangled.nextIf('a')) {
                auto tableTemplate = Factory.createNode(Node::Kind::ProtocolWitnessTableAccessor);
                DEMANGLE_CHILD_OR_RETURN(tableTemplate, ProtocolConformance);
                return tableTemplate;
            }
            if (Mangled.nextIf('t')) {
                auto accessor = Factory.createNode(Node::Kind::AssociatedTypeMetadataAccessor);
                DEMANGLE_CHILD_OR_RETURN(accessor, ProtocolConformance);
                DEMANGLE_CHILD_OR_RETURN(accessor, DeclName);
                return accessor;
            }
            if (Mangled.nextIf('T')) {
                auto accessor = Factory.createNode(Node::Kind::AssociatedTypeWitnessTableAccessor);
                DEMANGLE_CHILD_OR_RETURN(accessor, ProtocolConformance);
                DEMANGLE_CHILD_OR_RETURN(accessor, DeclName);
    ...
    NodePointer
    demangleGenericSpecialization(NodePointer specialization)
    {
...
#define FUNCSIGSPEC_CREATE_PARAM_PAYLOAD(payload)                                                  \
    Factory.createNode(Node::Kind::FunctionSignatureSpecializationParamPayload, payload)

    bool
    demangleFuncSigSpecializationConstantProp(NodePointer parent)
    {
    ...
    bool
    demangleFuncSigSpecializationClosureProp(NodePointer parent)
    {
...
#undef FUNCSIGSPEC_CREATE_PARAM_PAYLOAD
    ...
    demangleSpecializedAttribute()
    {
        ...
        if (Mangled.nextIf("g") || (isNotReAbstracted = Mangled.nextIf("r"))) {
            ...
            spec->addChild(
                Factory.createNode(Node::Kind::SpecializationPassID, unsigned(Mangled.next() - 48)),
                Factory);
        ...
        if (Mangled.nextIf("f")) {
            auto spec = Factory.createNode(Node::Kind::FunctionSignatureSpecialization);
    ...
    NodePointer
    demangleIdentifier(llvm::Optional<Node::Kind> kind = llvm::None)
    {
        if (!Mangled)
            return nullptr;
    ...
    bool
    demangleIndex(Node::IndexType &natural)
    {
        if (Mangled.nextIf('_')) {
            natural = 0;
            return true;
    ...
    NodePointer
    demangleIndexAsNode(Node::Kind kind = Node::Kind::Number)
    {
    ...
    NodePointer
    createSwiftType(Node::Kind typeKind, StringRef name)
    {
    ...
    demangleSubstitutionIndex()
    {
        ...
        if (Mangled.nextIf('C'))
            return Factory.createNode(Node::Kind::Module, MANGLING_MODULE_CLANG_IMPORTER);
        if (Mangled.nextIf('a'))
    ...
    NodePointer
    demangleDeclarationName(Node::Kind kind)
    {
    ...
    NodePointer
    demangleProtocolNameGivenContext(NodePointer context)
    {
    ...
    NodePointer
    demangleBoundGenericArgs(NodePointer nominalType)
    {
    ...
    demangleProtocolConformance()
    {
        ...
        if (!context)
            return nullptr;
        NodePointer proto_conformance = Factory.createNode(Node::Kind::ProtocolConformance);
    ...
    demangleEntity()
    {
        ...
        if (wrapEntity) {
            ...
            case Node::Kind::PrivateDeclName: // identifier file-discriminator?
                if (name->getNumChildren() > 1 && name->getChild(1)->getText() == "subscript") {
                    isSubscript = true;
    ...
    NodePointer
    getDependentGenericParamType(unsigned depth, unsigned index)
    {
    ...
    NodePointer
    demangleDependentMemberTypeName(NodePointer base)
    {
    ...
    NodePointer
    demangleAssociatedTypeCompound()
    {
        // Demangle the base type.
        auto base = demangleGenericParamIndex();
        if (!base)
    ...
    NodePointer
    demangleGenericSignature(bool isPseudogeneric = false)
    {
        auto sig = Factory.createNode(isPseudogeneric ? Node::Kind::DependentPseudogenericSignature
                                                      : Node::Kind::DependentGenericSignature);
        // First read in the parameter counts at each depth.
        Node::IndexType count = ~(Node::IndexType)0;

        auto addCount = [&] {
            auto countNode = Factory.createNode(Node::Kind::DependentGenericParamCount, count);
    ...
    demangleMetatypeRepresentation()
    {
        ...
        if (Mangled.nextIf('o'))
            return Factory.createNode(Node::Kind::MetatypeRepresentation, "@objc_metatype");
    ...
    demangleGenericRequirement()
    {
        ...
        if (Mangled.nextIf('z')) {
            ...
            if (!second)
                return nullptr;
            auto reqt = Factory.createNode(Node::Kind::DependentGenericSameTypeRequirement);
        ...
        if (Mangled.nextIf('l')) {
            ...
            if (!second)
                return nullptr;
            auto reqt = Factory.createNode(Node::Kind::DependentGenericLayoutRequirement);
            reqt->addChild(constrainedType, Factory);
            reqt->addChild(second, Factory);
            if (size != SIZE_MAX) {
        ...
        auto reqt = Factory.createNode(Node::Kind::DependentGenericConformanceRequirement);
        reqt->addChild(constrainedType, Factory);
    ...
    NodePointer
    postProcessReturnTypeNode(NodePointer out_args)
    {
    ...
    NodePointer
    demangleFunctionType(Node::Kind kind)
    {
        ...
        if (Mangled) {
            ...
            if (Mangled.nextIf('D')) {
                switch (auto nkind = (MangledDifferentiabilityKind)Mangled.next()) {
        ...
        if (concurrent) {
            block->addChild(Factory.createNode(Node::Kind::ConcurrentFunctionType), Factory);
        }
        if (diffKind != MangledDifferentiabilityKind::NonDifferentiable) {
            block->addChild(
                Factory.createNode(Node::Kind::DifferentiableFunctionType, (char)diffKind),
                Factory);
        }
        if (globalActorType) {
            auto globalActorNode = Factory.createNode(Node::Kind::GlobalActorFunctionType);
    ...
    demangleTypeImpl()
    {
        ...
        if (c == 'B') {
            ...
            c = Mangled.next();
            if (c == 'b')
                return Factory.createNode(Node::Kind::BuiltinTypeName, "Builtin.BridgeObject");
            if (c == 'B')
                return Factory.createNode(Node::Kind::BuiltinTypeName, "Builtin.UnsafeValueBuffer");
            if (c == 'f') {
                Node::IndexType size;
                ...
                if (demangleNatural(elts)) {
                    ...
                    if (Mangled.nextIf('f')) {
                        ...
                        if (!demangleBuiltinSize(size))
                            return nullptr;
                        return Factory.createNode(Node::Kind::BuiltinTypeName,
                            (DemanglerPrinter() << "Builtin.Vec" << elts << "xFloat" << size)
                                .str());
                    }
                    if (Mangled.nextIf('p'))
                        return Factory.createNode(Node::Kind::BuiltinTypeName,
                            (DemanglerPrinter() << "Builtin.Vec" << elts << "xRawPointer").str());
                }
            }
            if (c == 'O')
                return Factory.createNode(Node::Kind::BuiltinTypeName, "Builtin.UnknownObject");
            if (c == 'o')
                return Factory.createNode(Node::Kind::BuiltinTypeName, "Builtin.NativeObject");
            if (c == 'p')
                return Factory.createNode(Node::Kind::BuiltinTypeName, "Builtin.RawPointer");
            if (c == 't')
                return Factory.createNode(Node::Kind::BuiltinTypeName, "Builtin.SILToken");
            if (c == 'w')
                return Factory.createNode(Node::Kind::BuiltinTypeName, "Builtin.Word");
            return nullptr;
        }
        if (c == 'a')
        ...
        if (c == 'X') {
            ...
            if (Mangled.nextIf('B')) {
                ...
                NodePointer boxType = Factory.createNode(Node::Kind::SILBoxTypeWithLayout);
        ...
        if (c == 'u') {
            ...
            if (!sub)
                return nullptr;
            NodePointer dependentGenericType = Factory.createNode(Node::Kind::DependentGenericType);
    ...
    bool
    demangleReabstractSignature(NodePointer signature)
    {
        if (Mangled.nextIf('G')) {
    ...
    StringRef
    demangleImplConvention(ImplConventionContext ctxt)
    {
...
#undef CASE
    ...
    bool
    demangleImplCalleeConvention(NodePointer type)
    {
        StringRef attr;
        if (Mangled.nextIf('t')) {
    ...
    void
    addImplFunctionAttribute(
        NodePointer parent, StringRef attr, Node::Kind kind = Node::Kind::ImplFunctionAttribute)
    {
        parent->addChild(Factory.createNode(kind, attr), Factory);
    }

    void
    addImplFunctionConvention(NodePointer parent, StringRef attr)
    {
        auto attrNode = Factory.createNode(Node::Kind::ImplFunctionConvention);
        attrNode->addChild(
            Factory.createNode(Node::Kind::ImplFunctionConventionName, attr), Factory);
        parent->addChild(attrNode, Factory);
    }

    // impl-parameter ::= impl-convention type
    bool
    demangleImplParameters(NodePointer parent)
    {
        while (!Mangled.nextIf('_')) {
    ...
    bool
    demangleImplResults(NodePointer parent)
    {
        while (!Mangled.nextIf('_')) {
    ...
    NodePointer
    demangleImplParameterOrResult(Node::Kind kind)
    {
        if (Mangled.nextIf('z')) {
        ...
        } else if (kind == Node::Kind::ImplResult || kind == Node::Kind::ImplErrorResult) {
        ...
        node->addChild(Factory.createNode(Node::Kind::ImplConvention, convention), Factory);
...
NodePointer
swift::Demangle::demangleOldSymbolAsNode(StringRef MangledName, NodeFactory &Factory)
{
