...
unreachable(const char *Message)
{
...
static char
getCharOfNodeText(Node *node, unsigned idx)
{
    switch (node->getKind()) {
...
bool
SubstitutionEntry::identifierEquals(Node *lhs, Node *rhs)
{
...
void
SubstitutionEntry::deepHash(Node *node)
{
...
bool
SubstitutionEntry::deepEquals(Node *lhs, Node *rhs) const
{
    if (lhs->getKind() != rhs->getKind())
        return false;
    if (lhs->hasIndex()) {
    ...
    for (auto li = lhs->begin(), ri = rhs->begin(), le = lhs->end(); li != le; ++li, ++ri) {
        if (!deepEquals(*li, *ri))
            return false;
    }
...
int
RemanglerBase::findSubstitution(const SubstitutionEntry &entry)
{
    // First search in InlineSubstitutions.
    SubstitutionEntry *result
        = std::find(InlineSubstitutions, InlineSubstitutions + NumInlineSubsts, entry);
...
void
RemanglerBase::addSubstitution(const SubstitutionEntry &entry)
{
...
class Remangler : public RemanglerBase {
    template <typename Mangler> friend void Mangle::mangleIdentifier(Mangler &M, StringRef ident);
    ...
    void
    addSubstWordsInIdent(const WordReplacement &repl)
    {
        SubstWordsInIdent.push_back(repl, Factory);
    }

    void
    addWord(const SubstitutionWord &word)
    {
        Words.push_back(word, Factory);
    }

    template <typename Mangler> friend void mangleIdentifier(Mangler &M, StringRef ident);
    ...
    public:
        ...
        public:
            ManglingContextRAII(EntityContext &ctx)
                : Ctx(ctx)
                , SavedValue(ctx.AsContext)
            {
                ctx.AsContext = true;
            }

            ~ManglingContextRAII() { Ctx.AsContext = SavedValue; }
        };
    };
    ...
    Node *
    getSingleChild(Node *node, Node::Kind kind)
    {
    ...
    void
    mangleIndex(Node::IndexType value)
    {
        if (value == 0) {
            Buffer << '_';
        } else {
    ...
    void
    mangleChildNodes(Node *node)
    {
        mangleNodes(node->begin(), node->end());
    }
    void
    mangleChildNodesReversed(Node *node)
    {
    ...
    void
    mangleListSeparator(bool &isFirstListItem)
    {
        if (isFirstListItem) {
            Buffer << '_';
    ...
    void
    mangleEndOfList(bool isFirstListItem)
    {
        if (isFirstListItem)
            Buffer << 'y';
    }
    ...
    void
    mangleNodes(Node::iterator i, Node::iterator e)
    {
        for (; i != e; ++i) {
            mangle(*i);
        }
    ...
    void
    mangleSingleChildNode(Node *node)
    {
        assert(node->getNumChildren() == 1);
        mangle(*node->begin());
    }
    ...
    void
    mangleChildNode(Node *node, unsigned index)
    {
    ...
    void mangleProtocolList(Node *protocols, Node *superclass, bool hasExplicitAnyObject);

    bool trySubstitution(Node *node, SubstitutionEntry &entry, bool treatAsIdentifier = false);

    void mangleIdentifierImpl(Node *node, bool isOperator);

    bool mangleStandardSubstitution(Node *node);

    void mangleDependentGenericParamIndex(
        Node *node, const char *nonZeroPrefix = "", char zeroOp = 'z');

    std::pair<int, Node *> mangleConstrainedType(Node *node);

    void
    mangleFunctionSignature(Node *FuncType)
    {
    ...
    void mangleGenericArgs(Node *node, char &Separator, bool fullSubstitutionMap = false);
...
#define NODE(ID) void mangle##ID(Node *node);
...
bool
Remangler::trySubstitution(Node *node, SubstitutionEntry &entry, bool treatAsIdentifier)
{
...
void
Remangler::mangleIdentifierImpl(Node *node, bool isOperator)
{
    SubstitutionEntry entry;
    if (trySubstitution(node, entry, /*treatAsIdentifier*/ true))
        return;
    if (isOperator) {
        Mangle::mangleIdentifier(*this, Mangle::translateOperator(node->getText()));
    } else {
...
bool
Remangler::mangleStandardSubstitution(Node *node)
{
    if (node->getKind() != Node::Kind::Structure && node->getKind() != Node::Kind::Class
        && node->getKind() != Node::Kind::Enum && node->getKind() != Node::Kind::Protocol)
        return false;

    Node *context = node->getFirstChild();
    if (context->getKind() != Node::Kind::Module || context->getText() != STDLIB_NAME)
        return false;
...
void
Remangler::mangleDependentGenericParamIndex(Node *node, const char *nonZeroPrefix, char zeroOp)
{
...
std::pair<int, Node *>
Remangler::mangleConstrainedType(Node *node)
{
    if (node->getKind() == Node::Kind::Type)
...
void
Remangler::mangleAnyGenericType(Node *node, StringRef TypeOp)
{
    SubstitutionEntry entry;
    if (trySubstitution(node, entry))
        return;
...
void
Remangler::mangleAnyNominalType(Node *node)
{
    ...
    case Node::Kind::Structure:
        return mangleAnyGenericType(node, "V");
    case Node::Kind::Enum:
        return mangleAnyGenericType(node, "O");
    case Node::Kind::Class:
        return mangleAnyGenericType(node, "C");
    case Node::Kind::OtherNominalType:
        return mangleAnyGenericType(node, "XY");
    case Node::Kind::TypeAlias:
        return mangleAnyGenericType(node, "a");
    default:
...
void
Remangler::mangleGenericArgs(Node *node, char &Separator, bool fullSubstitutionMap)
{
    switch (node->getKind()) {
    ...
    case Node::Kind::TypeAlias:
        ...
        mangleGenericArgs(node->getChild(0), Separator, fullSubstitutionMap);
        Buffer << Separator;
        Separator = '_';
        break;
    ...
    case Node::Kind::BoundGenericTypeAlias: {
        ...
        mangleGenericArgs(parentOrModule, Separator, fullSubstitutionMap);
        Buffer << Separator;
        Separator = '_';
    ...
    case Node::Kind::Extension:
        mangleGenericArgs(node->getChild(1), Separator, fullSubstitutionMap);
        break;
...
void
Remangler::mangleAbstractStorage(Node *node, StringRef accessorCode)
{
    mangleChildNodes(node);
    switch (node->getKind()) {
    case Node::Kind::Subscript:
...
void
Remangler::mangleAllocator(Node *node)
{
    mangleAnyConstructor(node, 'C');
}

void
Remangler::mangleArgumentTuple(Node *node)
{
    Node *Child = skipType(getSingleChild(node));
    if (Child->getKind() == Node::Kind::Tuple && Child->getNumChildren() == 0) {
        Buffer << 'y';
        return;
    }
    mangle(Child);
...
void
Remangler::mangleAssociatedType(Node *node)
{
    unreachable("unsupported node");
}

void
Remangler::mangleAssociatedTypeRef(Node *node)
{
    SubstitutionEntry entry;
    if (trySubstitution(node, entry))
        return;
...
void
Remangler::mangleAssociatedTypeDescriptor(Node *node)
{
    mangleChildNodes(node);
    Buffer << "Tl";
}
...
void
Remangler::mangleAssociatedConformanceDescriptor(Node *node)
{
...
void
Remangler::mangleBaseConformanceDescriptor(Node *node)
{
    mangle(node->getChild(0));
...
void
Remangler::mangleAssociatedTypeMetadataAccessor(Node *node)
{
...
void
Remangler::mangleBaseWitnessTableAccessor(Node *node)
{
...
void
Remangler::mangleAutoClosureType(Node *node)
{
...
void
Remangler::mangleEscapingAutoClosureType(Node *node)
{
    mangleChildNodesReversed(node); // argument tuple, result type
    Buffer << "XA";
}
...
void
Remangler::mangleNoEscapeFunctionType(Node *node)
{
    mangleChildNodesReversed(node); // argument tuple, result type
    Buffer << "XE";
}
...
void
Remangler::mangleBoundGenericClass(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleBoundGenericEnum(Node *node)
{
...
void
Remangler::mangleBoundGenericStructure(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleBoundGenericOtherNominalType(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleBoundGenericProtocol(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleBoundGenericTypeAlias(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleBoundGenericFunction(Node *node)
{
    SubstitutionEntry entry;
    if (trySubstitution(node, entry))
        return;
...
void
Remangler::mangleBuiltinTypeName(Node *node)
{
    Buffer << 'B';
...
void
Remangler::mangleCFunctionPointer(Node *node)
{
    if (node->getNumChildren() > 0 && node->getFirstChild()->getKind() == Node::Kind::ClangType) {
...
void
Remangler::mangleClass(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleAnyConstructor(Node *node, char kindOp)
{
    mangleChildNodes(node);
    Buffer << "f" << kindOp;
}
...
void
Remangler::mangleConstructor(Node *node)
{
    mangleAnyConstructor(node, 'c');
}

void
Remangler::mangleCoroutineContinuationPrototype(Node *node)
{
    mangleChildNodes(node);
    Buffer << "TC";
}
...
void
Remangler::mangleObjCAsyncCompletionHandlerImpl(Node *node)
{
...
void
Remangler::mangleDeallocator(Node *node)
{
    mangleChildNodes(node);
    Buffer << "fD";
}
...
void
Remangler::mangleDeclContext(Node *node)
{
    mangleSingleChildNode(node);
}

void
Remangler::mangleDefaultArgumentInitializer(Node *node)
{
    mangleChildNode(node, 0);
    Buffer << "fA";
    mangleChildNode(node, 1);
...
void
Remangler::mangleAsyncFunctionPointer(Node *node)
{
    Buffer << "Tu";
}

void
Remangler::mangleDependentAssociatedTypeRef(Node *node)
{
...
void
Remangler::mangleDependentGenericParamCount(Node *node)
{
    unreachable("handled inline in DependentGenericSignature");
}

void
Remangler::mangleDependentGenericParamType(Node *node)
{
    if (node->getChild(0)->getIndex() == 0 && node->getChild(1)->getIndex() == 0) {
        Buffer << 'x';
        return;
    }
    Buffer << 'q';
...
void
Remangler::mangleDependentGenericLayoutRequirement(Node *node)
{
    auto NumMembersAndParamIdx = mangleConstrainedType(node->getChild(0));
    assert(NumMembersAndParamIdx.first < 0 || NumMembersAndParamIdx.second);
    switch (NumMembersAndParamIdx.first) {
...
void
Remangler::mangleDependentGenericSignature(Node *node)
{
...
void
Remangler::mangleDependentGenericType(Node *node)
{
...
void
Remangler::mangleDependentMemberType(Node *node)
{
...
void
Remangler::mangleDependentPseudogenericSignature(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleDestructor(Node *node)
{
    mangleChildNodes(node);
    Buffer << "fd";
}
...
void
Remangler::mangleDidSet(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "W");
}

void
Remangler::mangleDirectness(Node *node)
{
...
void
Remangler::mangleDynamicAttribute(Node *node)
{
    Buffer << "TD";
}

void
Remangler::mangleDirectMethodReferenceAttribute(Node *node)
{
    Buffer << "Td";
}

void
Remangler::mangleDynamicSelf(Node *node)
{
...
void
Remangler::mangleEnum(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleErrorType(Node *node)
{
    Buffer << "Xe";
}

void
Remangler::mangleExistentialMetatype(Node *node)
{
...
void
Remangler::mangleExplicitClosure(Node *node)
{
...
void
Remangler::mangleExtension(Node *node)
{
    mangleChildNode(node, 1);
    mangleChildNode(node, 0);
    if (node->getNumChildren() == 3)
...
void
Remangler::mangleAnonymousContext(Node *node)
{
    mangleChildNode(node, 1);
    mangleChildNode(node, 0);
    if (node->getNumChildren() >= 3)
...
void
Remangler::mangleFieldOffset(Node *node)
{
...
void
Remangler::mangleEnumCase(Node *node)
{
...
void
Remangler::mangleFullTypeMetadata(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mf";
}
...
void
Remangler::mangleFunction(Node *node)
{
    mangleChildNode(node, 0); // context
...
void
Remangler::mangleFunctionSignatureSpecialization(Node *node)
{
    ...
    for (NodePointer Child : *node) {
        ...
        if (Child->getKind() == Node::Kind::SpecializationPassID && node->hasIndex()) {
...
Remangler::mangleFunctionSignatureSpecializationParam(Node *node)
{
    ...
    case FunctionSigSpecializationParamKind::SROA:
        ...
        return;
    default:
        if (kindValue & unsigned(FunctionSigSpecializationParamKind::ExistentialToGeneric)) {
            Buffer << 'e';
            if (kindValue & unsigned(FunctionSigSpecializationParamKind::Dead))
                Buffer << 'D';
            if (kindValue & unsigned(FunctionSigSpecializationParamKind::OwnedToGuaranteed))
                Buffer << 'G';
            if (kindValue & unsigned(FunctionSigSpecializationParamKind::GuaranteedToOwned))
                Buffer << 'O';
        } else if (kindValue & unsigned(FunctionSigSpecializationParamKind::Dead)) {
            Buffer << 'd';
            if (kindValue & unsigned(FunctionSigSpecializationParamKind::OwnedToGuaranteed))
                Buffer << 'G';
            if (kindValue & unsigned(FunctionSigSpecializationParamKind::GuaranteedToOwned))
                Buffer << 'O';
        } else if (kindValue & unsigned(FunctionSigSpecializationParamKind::OwnedToGuaranteed)) {
            Buffer << 'g';
        } else if (kindValue & unsigned(FunctionSigSpecializationParamKind::GuaranteedToOwned)) {
            Buffer << 'o';
        }
...
void
Remangler::mangleFunctionType(Node *node)
{
...
void
Remangler::mangleGenericProtocolWitnessTable(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "WG";
}
...
void
Remangler::mangleGenericProtocolWitnessTableInstantiationFunction(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "WI";
}
...
void
Remangler::mangleResilientProtocolWitnessTable(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Wr";
}
...
void
Remangler::mangleGenericPartialSpecialization(Node *node)
{
    for (NodePointer Child : *node) {
    ...
    Buffer << (node->getKind() == Node::Kind::GenericPartialSpecializationNotReAbstracted ? "TP"
                                                                                          : "Tp");
    for (NodePointer Child : *node) {
        if (Child->getKind() != Node::Kind::GenericSpecializationParam)
            mangle(Child);
...
void
Remangler::mangleGenericSpecialization(Node *node)
{
...
void
Remangler::mangleInlinedGenericFunction(Node *node)
{
    mangleGenericSpecializationNode(node, "Ti");
}

void
Remangler::mangleGenericSpecializationParam(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleGenericTypeMetadataPattern(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "MP";
}
...
void
Remangler::mangleGenericTypeParamDecl(Node *node)
{
    mangleChildNodes(node);
    Buffer << "fp";
}
...
void
Remangler::mangleGetter(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "g");
}

void
Remangler::mangleGlobal(Node *node)
{
...
void
Remangler::mangleGlobalGetter(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "G");
}

void
Remangler::mangleIdentifier(Node *node)
{
    mangleIdentifierImpl(node, /*isOperator*/ false);
}

void
Remangler::mangleIndex(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleUnknownIndex(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleIVarInitializer(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "fe";
}
...
void
Remangler::mangleIVarDestroyer(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "fE";
}
...
void
Remangler::mangleImplDifferentiabilityKind(Node *node)
{
    Buffer << (char)node->getIndex();
}

void
Remangler::mangleImplEscaping(Node *node)
{
    Buffer << 'e';
}

void
Remangler::mangleImplConvention(Node *node)
{
...
Remangler::mangleImplParameterResultDifferentiability(Node *node)
{
    ...
    if (node->getText().empty())
        return;
    char diffChar = llvm::StringSwitch<char>(node->getText()).Case("@noDerivative", 'w').Default(0);
    assert(diffChar && "Invalid impl differentiability");
    Buffer << diffChar;
}

void
Remangler::mangleImplFunctionAttribute(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleImplFunctionConvention(Node *node)
{
    StringRef text = (node->getNumChildren() > 0 && node->getFirstChild()->hasText())
...
void
Remangler::mangleImplFunctionConventionName(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleClangType(Node *node)
{
    Buffer << node->getText().size() << node->getText();
}

void
Remangler::mangleImplInvocationSubstitutions(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleImplPatternSubstitutions(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleImplFunctionType(Node *node)
{
...
void
Remangler::mangleImplicitClosure(Node *node)
{
    mangleChildNode(node, 0); // context
    mangleChildNode(node, 2); // type
    Buffer << "fu";
...
void
Remangler::mangleImplParameter(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleImplResult(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleImplYield(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleImplErrorResult(Node *node)
{
    unreachable("handled inline");
}

void
Remangler::mangleInOut(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << 'z';
}
...
void
Remangler::mangleIsolated(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Yi";
}
...
void
Remangler::mangleShared(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << 'h';
}
...
void
Remangler::mangleOwned(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << 'n';
}
...
void
Remangler::mangleNoDerivative(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Yk";
}
...
void
Remangler::mangleInfixOperator(Node *node)
{
...
void
Remangler::mangleInitializer(Node *node)
{
    mangleChildNodes(node);
    Buffer << "fi";
}
...
void
Remangler::manglePropertyWrapperBackingInitializer(Node *node)
{
    mangleChildNodes(node);
    Buffer << "fP";
}
...
void
Remangler::mangleLazyProtocolWitnessTableAccessor(Node *node)
{
    mangleChildNodes(node);
    Buffer << "Wl";
}
...
void
Remangler::mangleLocalDeclName(Node *node)
{
...
void
Remangler::mangleMaterializeForSet(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "m");
}

void
Remangler::mangleMetatype(Node *node)
{
    if (node->getFirstChild()->getKind() == Node::Kind::MetatypeRepresentation) {
        mangleChildNode(node, 1);
        Buffer << "XM";
...
void
Remangler::mangleMetatypeRepresentation(Node *node)
{
...
void
Remangler::mangleMetaclass(Node *node)
{
    mangleChildNodes(node);
    Buffer << "Mm";
}
...
void
Remangler::mangleModifyAccessor(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "M");
}

void
Remangler::mangleModule(Node *node)
{
...
void
Remangler::mangleNativeOwningAddressor(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "lo");
}

void
Remangler::mangleNativeOwningMutableAddressor(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "ao");
}

void
Remangler::mangleNativePinningAddressor(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "lp");
}

void
Remangler::mangleNativePinningMutableAddressor(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "aP");
}

void
Remangler::mangleClassMetadataBaseOffset(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mo";
}
...
void
Remangler::mangleNominalTypeDescriptor(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mn";
}
...
void
Remangler::mangleOpaqueTypeDescriptor(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "MQ";
}
...
void
Remangler::mangleOpaqueTypeDescriptorAccessor(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mg";
}
...
void
Remangler::mangleOpaqueTypeDescriptorAccessorImpl(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mh";
}
...
void
Remangler::mangleOpaqueTypeDescriptorAccessorKey(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mj";
}
...
void
Remangler::mangleOpaqueTypeDescriptorAccessorVar(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mk";
}
...
void
Remangler::manglePropertyDescriptor(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "MV";
}
...
void
Remangler::mangleNonObjCAttribute(Node *node)
{
    Buffer << "TO";
}

void
Remangler::mangleTuple(Node *node)
{
    mangleTypeList(node);
    Buffer << 't';
}
...
void
Remangler::mangleNumber(Node *node)
{
    mangleIndex(node->getIndex());
}

void
Remangler::mangleObjCAttribute(Node *node)
{
    Buffer << "To";
}

void
Remangler::mangleObjCBlock(Node *node)
{
    if (node->getNumChildren() > 0 && node->getFirstChild()->getKind() == Node::Kind::ClangType) {
        for (size_t Idx = node->getNumChildren() - 1; Idx >= 1; --Idx) {
            mangleChildNode(node, Idx);
...
void
Remangler::mangleEscapingObjCBlock(Node *node)
{
    mangleChildNodesReversed(node);
    Buffer << "XL";
}
...
void
Remangler::mangleOwningAddressor(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "lO");
}

void
Remangler::mangleOwningMutableAddressor(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "aO");
}

void
Remangler::manglePartialApplyForwarder(Node *node)
{
    mangleChildNodesReversed(node);
    Buffer << "TA";
}
...
void
Remangler::manglePartialApplyObjCForwarder(Node *node)
{
    mangleChildNodesReversed(node);
    Buffer << "Ta";
}
...
void
Remangler::mangleMergedFunction(Node *node)
{
    Buffer << "Tm";
}
...
void
Remangler::mangleDynamicallyReplaceableFunctionKey(Node *node)
{
    Buffer << "Tx";
}

void
Remangler::mangleDynamicallyReplaceableFunctionVar(Node *node)
{
    Buffer << "TX";
}

void
Remangler::mangleAsyncAwaitResumePartialFunction(Node *node)
{
    Buffer << "TQ";
    mangleChildNode(node, 0);
}
...
void
Remangler::mangleAsyncSuspendResumePartialFunction(Node *node)
{
    Buffer << "TY";
    mangleChildNode(node, 0);
}
...
void
Remangler::manglePostfixOperator(Node *node)
{
    mangleIdentifierImpl(node, /*isOperator*/ true);
    Buffer << "oP";
}
...
void
Remangler::manglePrefixOperator(Node *node)
{
    mangleIdentifierImpl(node, /*isOperator*/ true);
    Buffer << "op";
}
...
void
Remangler::manglePrivateDeclName(Node *node)
{
    mangleChildNodesReversed(node);
...
void
Remangler::mangleProtocol(Node *node)
{
    mangleAnyGenericType(node, "P");
}

void
Remangler::mangleRetroactiveConformance(Node *node)
{
...
void
Remangler::mangleProtocolConformance(Node *node)
{
...
void
Remangler::mangleConcreteProtocolConformance(Node *node)
{
...
void
Remangler::mangleDependentProtocolConformanceRoot(Node *node)
{
    mangleType(node->getChild(0));
    manglePureProtocol(node->getChild(1));
    Buffer << "HD";
...
void
Remangler::mangleDependentAssociatedConformance(Node *node)
{
    mangleType(node->getChild(0));
    manglePureProtocol(node->getChild(1));
}
...
void
Remangler::mangleDependentConformanceIndex(Node *node)
{
    assert(node->getKind() == Node::Kind::Index || node->getKind() == Node::Kind::UnknownIndex);
    assert(node->hasIndex() == (node->getKind() == Node::Kind::Index));
    mangleIndex(node->hasIndex() ? node->getIndex() + 2 : 1);
}

void
Remangler::mangleAnyProtocolConformance(Node *node)
{
    switch (node->getKind()) {
...
void
Remangler::mangleAnyProtocolConformanceList(Node *node)
{
    bool firstElem = true;
    for (NodePointer child : *node) {
...
void
Remangler::mangleProtocolDescriptor(Node *node)
{
...
void
Remangler::mangleProtocolSelfConformanceDescriptor(Node *node)
{
    manglePureProtocol(node->getChild(0));
    Buffer << "MS";
}
...
void
Remangler::mangleProtocolConformanceDescriptor(Node *node)
{
...
void
Remangler::mangleProtocolList(Node *node, Node *superclass, bool hasExplicitAnyObject)
{
...
void
Remangler::mangleProtocolList(Node *node)
{
    mangleProtocolList(node, nullptr, false);
}

void
Remangler::mangleProtocolListWithClass(Node *node)
{
    mangleProtocolList(node->getChild(0), node->getChild(1), false);
}

void
Remangler::mangleProtocolListWithAnyObject(Node *node)
{
    mangleProtocolList(node->getChild(0), nullptr, true);
}

void
Remangler::mangleProtocolSelfConformanceWitness(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "TS";
}
...
void
Remangler::mangleProtocolWitness(Node *node)
{
    mangleChildNodes(node);
    Buffer << "TW";
}
...
void
Remangler::mangleProtocolWitnessTable(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "WP";
}
...
void
Remangler::mangleProtocolWitnessTablePattern(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Wp";
}
...
void
Remangler::mangleProtocolWitnessTableAccessor(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Wa";
}
...
void
Remangler::mangleReabstractionThunk(Node *node)
{
    mangleChildNodesReversed(node);
    Buffer << "Tr";
}
...
void
Remangler::mangleReabstractionThunkHelper(Node *node)
{
    mangleChildNodesReversed(node);
    Buffer << "TR";
}
...
void
Remangler::mangleReabstractionThunkHelperWithSelf(Node *node)
{
    mangleChildNodesReversed(node);
    Buffer << "Ty";
}
...
void
Remangler::mangleAutoDiffFunctionOrSimpleThunk(Node *node, StringRef op)
{
    auto childIt = node->begin();
    while (childIt != node->end() && (*childIt)->getKind() != Node::Kind::AutoDiffFunctionKind)
        mangle(*childIt++);
    Buffer << op;
    mangle(*childIt++); // kind
...
void
Remangler::mangleAutoDiffFunction(Node *node)
{
    mangleAutoDiffFunctionOrSimpleThunk(node, "TJ");
}

void
Remangler::mangleAutoDiffDerivativeVTableThunk(Node *node)
{
...
void
Remangler::mangleAutoDiffSubsetParametersThunk(Node *node)
{
    auto childIt = node->begin();
    while (childIt != node->end() && (*childIt)->getKind() != Node::Kind::AutoDiffFunctionKind)
        mangle(*childIt++);
...
void
Remangler::mangleAutoDiffFunctionKind(Node *node)
{
    Buffer << (char)node->getIndex();
}

void
Remangler::mangleDifferentiabilityWitness(Node *node)
{
    auto childIt = node->begin();
    while (childIt != node->end() && (*childIt)->getKind() != Node::Kind::Index)
        mangle(*childIt++);
    if (node->getLastChild()->getKind() == Node::Kind::DependentGenericSignature)
...
void
Remangler::mangleIndexSubset(Node *node)
{
    Buffer << node->getText();
}

void
Remangler::mangleReadAccessor(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "r");
}

void
Remangler::mangleKeyPathThunkHelper(Node *node, StringRef op)
{
    for (NodePointer Child : *node)
...
void
Remangler::mangleKeyPathGetterThunkHelper(Node *node)
{
    mangleKeyPathThunkHelper(node, "TK");
}

void
Remangler::mangleKeyPathSetterThunkHelper(Node *node)
{
    mangleKeyPathThunkHelper(node, "Tk");
}

void
Remangler::mangleKeyPathEqualsThunkHelper(Node *node)
{
    mangleKeyPathThunkHelper(node, "TH");
}

void
Remangler::mangleKeyPathHashThunkHelper(Node *node)
{
    mangleKeyPathThunkHelper(node, "Th");
}

void
Remangler::mangleReturnType(Node *node)
{
    mangleArgumentTuple(node);
}

void
Remangler::mangleRelatedEntityDeclName(Node *node)
{
    mangleChildNode(node, 1);
...
void
Remangler::mangleSILBoxType(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Xb";
}
...
void
Remangler::mangleSetter(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "s");
}

void
Remangler::mangleSpecializationPassID(Node *node)
{
    Buffer << node->getIndex();
}

void
Remangler::mangleIsSerialized(Node *node)
{
    Buffer << 'q';
}

void
Remangler::mangleStatic(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << 'Z';
}
...
void
Remangler::mangleOtherNominalType(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleStructure(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleSubscript(Node *node)
{
    mangleAbstractStorage(node, "p");
}

void
Remangler::mangleSuffix(Node *node)
{
...
void
Remangler::mangleThinFunctionType(Node *node)
{
    mangleFunctionSignature(node);
    Buffer << "Xf";
}
...
void
Remangler::mangleTupleElement(Node *node)
{
    mangleChildNodesReversed(node); // tuple type, element name?
}

void
Remangler::mangleTupleElementName(Node *node)
{
    mangleIdentifier(node);
}

void
Remangler::mangleType(Node *node)
{
    mangleSingleChildNode(node);
}

void
Remangler::mangleTypeAlias(Node *node)
{
    mangleAnyNominalType(node);
}

void
Remangler::mangleTypeList(Node *node)
{
    bool FirstElem = true;
    for (size_t Idx = 0, Num = node->getNumChildren(); Idx < Num; ++Idx) {
        mangleChildNode(node, Idx);
...
void
Remangler::mangleLabelList(Node *node)
{
...
void
Remangler::mangleTypeMangling(Node *node)
{
    mangleChildNodes(node);
    Buffer << 'D';
}
...
void
Remangler::mangleTypeMetadata(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "N";
}
...
void
Remangler::mangleTypeMetadataAccessFunction(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Ma";
}
...
void
Remangler::mangleTypeMetadataInstantiationCache(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "MI";
}
...
void
Remangler::mangleTypeMetadataInstantiationFunction(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mi";
}
...
void
Remangler::mangleTypeMetadataCompletionFunction(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mr";
}
...
void
Remangler::mangleTypeMetadataDemanglingCache(Node *node)
{
    mangleChildNodes(node);
    Buffer << "MD";
}
...
void
Remangler::mangleTypeMetadataLazyCache(Node *node)
{
    mangleChildNodes(node);
    Buffer << "ML";
}
...
void
Remangler::mangleUncurriedFunctionType(Node *node)
{
    mangleFunctionSignature(node);
...
void
Remangler::mangleValueWitnessTable(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "WV";
}
...
void
Remangler::mangleVariable(Node *node)
{
    mangleAbstractStorage(node, "p");
}

void
Remangler::mangleVTableAttribute(Node *node)
{
    unreachable("Old-fashioned vtable thunk in new mangling format");
}

void
Remangler::mangleVTableThunk(Node *node)
{
    mangleChildNodes(node);
    Buffer << "TV";
}
...
void
Remangler::mangleWillSet(Node *node)
{
    mangleAbstractStorage(node->getFirstChild(), "w");
}
...
void
Remangler::mangleReflectionMetadataFieldDescriptor(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "MF";
}
...
void
Remangler::mangleCurryThunk(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Tc";
}
...
void
Remangler::mangleDispatchThunk(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Tj";
}
...
void
Remangler::mangleMethodDescriptor(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Tq";
}
...
void
Remangler::mangleMethodLookupFunction(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mu";
}
...
void
Remangler::mangleObjCMetadataUpdateFunction(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "MU";
}
...
void
Remangler::mangleObjCResilientClassStub(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Ms";
}
...
void
Remangler::mangleFullObjCResilientClassStub(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mt";
}
...
void
Remangler::mangleConcurrentFunctionType(Node *node)
{
    Buffer << "Yb";
}

void
Remangler::mangleAsyncAnnotation(Node *node)
{
    Buffer << "Ya";
}

void
Remangler::mangleDifferentiableFunctionType(Node *node)
{
    Buffer << "Yj" << (char)node->getIndex(); // differentiability kind
}

void
Remangler::mangleGlobalActorFunctionType(Node *node)
{
    mangleChildNodes(node);
    Buffer << "Yc";
}
...
void
Remangler::mangleThrowsAnnotation(Node *node)
{
    Buffer << 'K';
}

void
Remangler::mangleEmptyList(Node *node)
{
    Buffer << 'y';
}

void
Remangler::mangleFirstElementMarker(Node *node)
{
    Buffer << '_';
}

void
Remangler::mangleVariadicMarker(Node *node)
{
    Buffer << 'd';
}

void
Remangler::mangleOutlinedCopy(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WOy";
}
...
void
Remangler::mangleOutlinedConsume(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WOe";
}
...
void
Remangler::mangleOutlinedRetain(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WOr";
}
...
void
Remangler::mangleOutlinedRelease(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WOs";
}
...
void
Remangler::mangleOutlinedInitializeWithTake(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WOb";
}
...
void
Remangler::mangleOutlinedInitializeWithCopy(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WOc";
}
...
void
Remangler::mangleOutlinedAssignWithTake(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WOd";
}
...
void
Remangler::mangleOutlinedAssignWithCopy(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WOf";
}
...
void
Remangler::mangleOutlinedDestroy(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WOh";
}
...
void
Remangler::mangleOutlinedVariable(Node *node)
{
    Buffer << "Tv";
    mangleIndex(node->getIndex());
}
...
void
Remangler::mangleOutlinedBridgedMethod(Node *node)
{
    Buffer << "Te";
    Buffer << node->getText();
    Buffer << "_";
...
void
Remangler::mangleSILBoxTypeWithLayout(Node *node)
{
    ...
    for (unsigned i = 0, e = layout->getNumChildren(); i < e; ++i) {
        ...
        assert(field->getNumChildren() == 1 && field->getChild(0)->getKind() == Node::Kind::Type);
...
void
Remangler::mangleSILBoxLayout(Node *node)
{
    unreachable("should be part of SILBoxTypeWithLayout");
}

void
Remangler::mangleSILBoxMutableField(Node *node)
{
    unreachable("should be part of SILBoxTypeWithLayout");
}

void
Remangler::mangleSILBoxImmutableField(Node *node)
{
    unreachable("should be part of SILBoxTypeWithLayout");
}

void
Remangler::mangleAssocTypePath(Node *node)
{
    bool FirstElem = true;
    for (NodePointer Child : *node) {
        mangle(Child);
...
void
Remangler::mangleModuleDescriptor(Node *node)
{
    mangle(node->getChild(0));
    Buffer << "MXM";
}
...
void
Remangler::mangleExtensionDescriptor(Node *node)
{
    mangle(node->getChild(0));
    Buffer << "MXE";
}
...
void
Remangler::mangleAnonymousDescriptor(Node *node)
{
    mangle(node->getChild(0));
    if (node->getNumChildren() == 1) {
        Buffer << "MXX";
...
void
Remangler::mangleAssociatedTypeGenericParamRef(Node *node)
{
    mangleType(node->getChild(0));
...
void
Remangler::mangleTypeSymbolicReference(Node *node)
{
    return mangle(Resolver(SymbolicReferenceKind::Context, (const void *)node->getIndex()));
}

void
Remangler::mangleProtocolSymbolicReference(Node *node)
{
    return mangle(Resolver(SymbolicReferenceKind::Context, (const void *)node->getIndex()));
}
...
void
Remangler::mangleSugaredOptional(Node *node)
{
    mangleType(node->getChild(0));
    Buffer << "XSq";
}
...
void
Remangler::mangleSugaredArray(Node *node)
{
    mangleType(node->getChild(0));
    Buffer << "XSa";
}
...
void
Remangler::mangleSugaredDictionary(Node *node)
{
    mangleType(node->getChild(0));
...
void
Remangler::mangleSugaredParen(Node *node)
{
    mangleType(node->getChild(0));
    Buffer << "XSp";
}
...
void
Remangler::mangleOpaqueReturnType(Node *node)
{
    Buffer << "Qr";
}
void
Remangler::mangleOpaqueReturnTypeOf(Node *node)
{
    mangle(node->getChild(0));
    Buffer << "QO";
}
void
Remangler::mangleOpaqueType(Node *node)
{
    SubstitutionEntry entry;
    if (trySubstitution(node, entry))
        return;
...
void
Remangler::mangleAccessorFunctionReference(Node *node)
{
...
void
Remangler::mangleCanonicalSpecializedGenericTypeMetadataAccessFunction(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mb";
}
...
void
Remangler::mangleMetadataInstantiationCache(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "MK";
}
...
void
Remangler::mangleNoncanonicalSpecializedGenericTypeMetadataCache(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "MJ";
}
...
void
Remangler::mangleCanonicalPrespecializedGenericTypeCachingOnceToken(Node *node)
{
    mangleSingleChildNode(node);
    Buffer << "Mz";
}
...
void
Remangler::mangleGlobalVariableOnceToken(Node *node)
{
    mangleChildNodes(node);
    Buffer << "Wz";
}
...
void
Remangler::mangleGlobalVariableOnceFunction(Node *node)
{
    mangleChildNodes(node);
    Buffer << "WZ";
}
...
void
Remangler::mangleGlobalVariableOnceDeclList(Node *node)
{
...
std::string
Demangle::mangleNode(NodePointer node)
{
...
llvm::StringRef
Demangle::mangleNode(NodePointer node, SymbolicResolver resolver, NodeFactory &Factory)
{
    if (!node)
        return StringRef();
...
bool
Demangle::isSpecialized(Node *node)
{
    switch (node->getKind()) {
    case Node::Kind::BoundGenericStructure:
    case Node::Kind::BoundGenericEnum:
...
NodePointer
Demangle::getUnspecialized(Node *node, NodeFactory &Factory)
{
