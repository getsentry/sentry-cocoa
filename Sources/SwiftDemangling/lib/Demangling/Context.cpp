...
namespace Demangle {
    ...
    NodePointer
    Context::demangleSymbolAsNode(llvm::StringRef MangledName)
    {
    ...
    NodePointer
    Context::demangleTypeAsNode(llvm::StringRef MangledName)
    {
        return D->demangleType(MangledName);
    }

    std::string
    Context::demangleSymbolAsString(llvm::StringRef MangledName, const DemangleOptions &Options)
    {
    ...
    std::string
    Context::demangleTypeAsString(llvm::StringRef MangledName, const DemangleOptions &Options)
    {
    ...
    stripSuffix(llvm::StringRef Name)
    {
        // A suffix always ends with a digit. Do this quick check to avoid scanning through the
        // whole symbol name if the symbol has no suffix (= the common case).
    ...
    bool
    Context::isThunkSymbol(llvm::StringRef MangledName)
    {
        if (isMangledName(MangledName)) {
            ...
            if (MangledName.endswith("TA") || // partial application forwarder
                ...
                if (!Nd || Nd->getKind() != Node::Kind::Global || Nd->getNumChildren() == 0)
                    return false;
    ...
    std::string
    Context::getThunkTarget(llvm::StringRef MangledName)
    {
        ...
        if (isMangledName(MangledName)) {
            ...
            if (MangledName.endswith("TR") || MangledName.endswith("Tr")
    ...
    bool
    Context::hasSwiftCallingConvention(llvm::StringRef MangledName)
    {
        Node *Global = demangleSymbolAsNode(MangledName);
        if (!Global || Global->getKind() != Node::Kind::Global || Global->getNumChildren() == 0)
            return false;
    ...
    std::string
    Context::getModuleName(llvm::StringRef mangledName)
    {
    ...
    demangleSymbolAsString(
        const char *MangledName, size_t MangledNameLength, const DemangleOptions &Options)
    {
        Context Ctx;
        return Ctx.demangleSymbolAsString(StringRef(MangledName, MangledNameLength), Options);
    }
    ...
    {
        Context Ctx;
        return Ctx.demangleTypeAsString(StringRef(MangledName, MangledNameLength), Options);
    }
