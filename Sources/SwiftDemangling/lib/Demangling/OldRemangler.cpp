...
public:
    ...
    public:
        ...
        void
        setAnonymousContextDiscriminator(StringRef discriminator)
        {
            AnonymousContextDiscriminator = discriminator.str();
        }

        std::string
        takeAnonymousContextDiscriminator()
        {
...
#include "swift/Demangling/DemangleNodes.def"
    ...
    void mangleAccessor(Node *storageNode, StringRef accessorCode, EntityContext &ctx);
    ...
    void mangleSimpleEntity(Node *node, char basicKind, StringRef entityKind, EntityContext &ctx);
    void mangleNamedEntity(Node *node, char basicKind, StringRef entityKind, EntityContext &ctx,
        StringRef ArtificialPrivateDiscriminator = {});
    void mangleTypedEntity(Node *node, char basicKind, StringRef entityKind, EntityContext &ctx);
    void mangleNamedAndTypedEntity(
        Node *node, char basicKind, StringRef entityKind, EntityContext &ctx);
    void mangleNominalType(Node *node, char basicKind, EntityContext &ctx,
        StringRef ArtificialPrivateDiscriminator = {});

    void mangleProtocolWithoutPrefix(Node *node);
    void mangleProtocolListWithoutPrefix(Node *node, Node *additionalProto = nullptr);
...
static NodePointer
applyParamLabels(NodePointer LabelList, NodePointer OrigType, NodeFactory &Factory)
{
    ...
    auto applyParamLabels = [&](NodePointer ArgTuple) -> NodePointer {
        ...
        for (unsigned i = 0, n = OrigTuple->getNumChildren(); i != n; ++i) {
            ...
            NewElt->addChild(
                Factory.createNodeWithAllocatedText(Node::Kind::TupleElementName, Label->getText()),
                Factory);
...
bool
Remangler::trySubstitution(Node *node, SubstitutionEntry &entry)
{
...
static bool
isInSwiftModule(Node *node)
{
    Node *context = node->getFirstChild();
    return (context->getKind() == Node::Kind::Module && context->getText() == STDLIB_NAME &&
...
void
Remangler::mangleIdentifier(StringRef ident, OperatorKind operatorKind)
{
...
void
Remangler::mangleIndex(Node::IndexType value)
{
    if (value == 0) {
        Buffer << '_';
    } else {
...
void
Remangler::mangleAutoDiffFunction(Node *node, EntityContext &ctx)
{
...
void
Remangler::mangleFunction(Node *node, EntityContext &ctx)
{
    mangleNamedAndTypedEntity(node, 'F', "", ctx);
}

void
Remangler::mangleVariable(Node *node, EntityContext &ctx)
{
    mangleNamedAndTypedEntity(node, 'v', "", ctx);
}

void
Remangler::mangleSubscript(Node *node, EntityContext &ctx)
{
    mangleNamedAndTypedEntity(node, 'i', "", ctx);
}

void
Remangler::mangleAccessor(Node *storageNode, StringRef accessorCode, EntityContext &ctx)
{
    Buffer << 'F';
...
void
Remangler::mangleInitializer(Node *node, EntityContext &ctx)
{
    mangleSimpleEntity(node, 'I', "i", ctx);
}

void
Remangler::manglePropertyWrapperBackingInitializer(Node *node, EntityContext &ctx)
{
    mangleSimpleEntity(node, 'I', "P", ctx);
}

void
Remangler::manglePropertyWrapperInitFromProjectedValue(Node *node, EntityContext &ctx)
{
    mangleSimpleEntity(node, 'I', "W", ctx);
}

void
Remangler::mangleDefaultArgumentInitializer(Node *node, EntityContext &ctx)
{
...
void
Remangler::mangleDeallocator(Node *node, EntityContext &ctx)
{
    mangleSimpleEntity(node, 'F', "D", ctx);
}

void
Remangler::mangleDestructor(Node *node, EntityContext &ctx)
{
    mangleSimpleEntity(node, 'F', "d", ctx);
}

void
Remangler::mangleAllocator(Node *node, EntityContext &ctx)
{
    mangleTypedEntity(node, 'F', "C", ctx);
}

void
Remangler::mangleConstructor(Node *node, EntityContext &ctx)
{
    mangleTypedEntity(node, 'F', "c", ctx);
}

void
Remangler::mangleIVarInitializer(Node *node, EntityContext &ctx)
{
    mangleSimpleEntity(node, 'F', "e", ctx);
}

void
Remangler::mangleIVarDestroyer(Node *node, EntityContext &ctx)
{
    mangleSimpleEntity(node, 'F', "E", ctx);
}

void
Remangler::mangleGetter(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "g", ctx);
}

void
Remangler::mangleGlobalGetter(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "G", ctx);
}

void
Remangler::mangleSetter(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "s", ctx);
}

void
Remangler::mangleMaterializeForSet(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "m", ctx);
}

void
Remangler::mangleWillSet(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "w", ctx);
}

void
Remangler::mangleDidSet(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "W", ctx);
}

void
Remangler::mangleOwningMutableAddressor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "aO", ctx);
}

void
Remangler::mangleNativeOwningMutableAddressor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "ao", ctx);
}

void
Remangler::mangleNativePinningMutableAddressor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "ap", ctx);
}

void
Remangler::mangleUnsafeMutableAddressor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "au", ctx);
}

void
Remangler::mangleOwningAddressor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "lO", ctx);
}

void
Remangler::mangleNativeOwningAddressor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "lo", ctx);
}

void
Remangler::mangleNativePinningAddressor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "lp", ctx);
}

void
Remangler::mangleUnsafeAddressor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "lu", ctx);
}

void
Remangler::mangleReadAccessor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "r", ctx);
}

void
Remangler::mangleModifyAccessor(Node *node, EntityContext &ctx)
{
    mangleAccessor(node->getFirstChild(), "M", ctx);
}

void
Remangler::mangleExplicitClosure(Node *node, EntityContext &ctx)
{
    mangleNamedAndTypedEntity(node, 'F', "U", ctx); // name is index
}

void
Remangler::mangleImplicitClosure(Node *node, EntityContext &ctx)
{
    mangleNamedAndTypedEntity(node, 'F', "u", ctx); // name is index
}

void
Remangler::mangleStatic(Node *node, EntityContext &ctx)
{
    Buffer << 'Z';
...
void
Remangler::mangleSimpleEntity(Node *node, char basicKind, StringRef entityKind, EntityContext &ctx)
{
    assert(node->getNumChildren() == 1);
    Buffer << basicKind;
...
void
Remangler::mangleNamedEntity(Node *node, char basicKind, StringRef entityKind, EntityContext &ctx,
    StringRef artificialPrivateDiscriminator)
{
    ...
    if (!artificialPrivateDiscriminator.empty())
        privateDiscriminator.append(
            artificialPrivateDiscriminator.data(), artificialPrivateDiscriminator.size());

    // Include the artificial private discriminator if one was given.
    auto name = node->getChild(1);
    if (!privateDiscriminator.empty() && name->getKind() == Node::Kind::Identifier) {
        Buffer << 'P';
...
void
Remangler::mangleTypedEntity(Node *node, char basicKind, StringRef entityKind, EntityContext &ctx)
{
...
void
Remangler::mangleNamedAndTypedEntity(
    Node *node, char basicKind, StringRef entityKind, EntityContext &ctx)
{
...
void
Remangler::mangleEntityContext(Node *node, EntityContext &ctx)
{
    ...
    switch (node->getKind()) {
#define NODE(ID) case Node::Kind::ID:
#define CONTEXT_NODE(ID)
#include "swift/Demangling/DemangleNodes.def"
...
void
Remangler::mangleEntityType(Node *node, EntityContext &ctx)
{
...
template <size_t N>
static bool
stripPrefix(StringRef &string, const char (&data)[N])
{
...
void
Remangler::mangleTypeAlias(Node *node, EntityContext &ctx)
{
    mangleAnyNominalType(node, ctx);
}
...
Remangler::mangleImplFunctionType(Node *node)
{
    ...
    for (; i != e && (*i)->getKind() == Node::Kind::ImplFunctionAttribute; ++i) {
    ...
    if (i != e
        ...
        Buffer << ((*i)->getKind() == Node::Kind::DependentGenericSignature ? 'G' : 'g');
...
void
Remangler::mangleProtocolListWithoutPrefix(Node *node, Node *additionalProto)
{
...
Remangler::mangleTuple(Node *node)
{
    ...
    if (NumElems > 0
        && node->getChild(NumElems - 1)->getFirstChild()->getKind() == Node::Kind::VariadicMarker) {
        Buffer << 't';
    } else {
        Buffer << 'T';
    }
...
Remangler::mangleDependentGenericSignature(Node *node)
{
    ...
    for (; i != e && (*i)->getKind() == Node::Kind::DependentGenericParamCount; ++i) {
        auto count = *i;
        if (count->getIndex() > 0)
...
Remangler::mangleDependentGenericConformanceRequirement(Node *node)
{
    ...
    if (node->getNumChildren() == 2 && node->getChild(1)->getKind() == Node::Kind::Type
...
{
    if (node->getFirstChild()->getKind() == Node::Kind::DependentGenericParamType) {
...
void
Remangler::mangleExtension(Node *node, EntityContext &ctx)
{
    assert(node->getNumChildren() == 2 || node->getNumChildren() == 3);
    if (node->getNumChildren() == 3) {
        Buffer << 'e';
...
void
Remangler::mangleAnonymousContext(Node *node, EntityContext &ctx)
{
...
void
Remangler::mangleModule(Node *node, EntityContext &ctx)
{
...
void
Remangler::mangleDependentGenericParamIndex(Node *node)
{
...
void
Remangler::mangleProtocol(Node *node, EntityContext &ctx)
{
    mangleNominalType(node, 'P', ctx);
}

void
Remangler::mangleProtocolWithoutPrefix(Node *node)
{
    if (mangleStandardSubstitution(node))
        return;
...
void
Remangler::mangleGenericArgs(Node *node, EntityContext &ctx)
{
    switch (node->getKind()) {
    case Node::Kind::Structure:
    case Node::Kind::Enum:
...
void
Remangler::mangleAnyNominalType(Node *node, EntityContext &ctx)
{
...
void
Remangler::mangleStructure(Node *node, EntityContext &ctx)
{
    mangleAnyNominalType(node, ctx);
}

void
Remangler::mangleEnum(Node *node, EntityContext &ctx)
{
    mangleAnyNominalType(node, ctx);
}

void
Remangler::mangleClass(Node *node, EntityContext &ctx)
{
    mangleAnyNominalType(node, ctx);
}

void
Remangler::mangleOtherNominalType(Node *node, EntityContext &ctx)
{
    mangleAnyNominalType(node, ctx);
}

void
Remangler::mangleNominalType(
    Node *node, char kind, EntityContext &ctx, StringRef artificialPrivateDiscriminator)
{
    SubstitutionEntry entry;
    if (trySubstitution(node, entry))
        return;
...
{
    Buffer << 'm';
    assert(node->getNumChildren() == 1 && node->getChild(0)->getKind() == Node::Kind::Type);
...
void
Remangler::mangleOpaqueReturnTypeOf(Node *node, EntityContext &ctx)
{
    unreachable("unsupported");
}
void
...
const char *
Demangle::mangleNodeAsObjcCString(NodePointer node, NodeFactory &Factory)
{
    assert(node);
