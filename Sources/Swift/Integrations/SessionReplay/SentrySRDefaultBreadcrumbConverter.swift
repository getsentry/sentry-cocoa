...
@_spi(Private) public class SentrySRDefaultBreadcrumbConverter: NSObject, SentryReplayBreadcrumbConverter {
    ...
    private func networkSpan(_ breadcrumb: Breadcrumb) -> SentryRRWebSpanEvent? {
        ...
        if let networkRequestData = breadcrumb.data?["_networkDetails"] as? SentryReplayNetworkDetails {
    ...
    private func addNetworkDetails(from networkRequestData: SentryReplayNetworkDetails, to data: inout [String: Any]) {
