...
///
/// `SentryNetworkTracker` creates this object and populates it via `setRequest`/`setResponse`;
/// `SentrySRDefaultBreadcrumbConverter` consumes it via `serialize()`.
///
/// - Important: The `@objc` capture and serialization methods must stay in the main class body,
///   not in an extension. `@objc` methods declared in a Swift extension compile to ObjC category
///   metadata that the static linker strips (without `-ObjC`/`-force_load`) in statically linked
///   distributions such as React Native, so the selectors are missing at runtime and the first
///   call to `setRequest` crashes with `unrecognized selector`. See
///   getsentry/sentry-react-native#6609.
///
...
@_spi(Private) public class SentryReplayNetworkDetails: NSObject {
    ...
    // MARK: - Capture
    //
    // These `@objc` methods must remain in the main class body. See the class-level note on why
    // moving them into an extension reintroduces getsentry/sentry-react-native#6609.
