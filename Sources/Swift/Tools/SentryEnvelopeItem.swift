...
@_spi(Private) @objc public final class SentryEnvelopeItem: NSObject {
    ...
    @objc public convenience init(event: Event) {
        guard let json = SentrySerialization.data(withJSONObject: event.serialize()) else {
            // We don't know what caused the serialization to fail.
            let errorEvent = SentryEvent(level: .warning)
            ...
            // error on purpose.
            let fallbackJson = SentrySerialization.data(withJSONObject: errorEvent.serialize()) ?? Data()
            ...
            let envelopeType = self.envelopeType(for: errorEvent)
            
            let itemHeader = SentryEnvelopeItemHeader(type: envelopeType, length: fallbackJson.count)
            self.init(header: itemHeader, data: fallbackJson)
            return
        }
        
        // event.type can be nil and the server infers error if there's a stack trace, otherwise
        // default. In any case in the envelope type it should be event. Except for transactions
        let envelopeType = self.envelopeType(for: event)
        
        let itemHeader = SentryEnvelopeItemHeader(type: envelopeType, length: UInt(json.count))
    ...
    @objc public convenience init(session: SentrySession) {
        guard let json = try? JSONSerialization.data(withJSONObject: session.serialize(), options: []) else {
            ...
            let itemHeader = SentryEnvelopeItemHeader(type: SentryEnvelopeItemTypeSession, length: 0)
            self.init(header: itemHeader, data: Data())
            return
        }
        
        let itemHeader = SentryEnvelopeItemHeader(type: SentryEnvelopeItemTypeSession, length: UInt(json.count))
        self.init(header: itemHeader, data: json)
    }
    ...
    @objc public convenience init(userFeedback: UserFeedback) {
        do {
            ...
            let itemHeader = SentryEnvelopeItemHeader(type: SentryEnvelopeItemTypeUserFeedback, length: UInt(json.count))
            self.init(header: itemHeader, data: json)
        } catch {
            SentrySDKLog.error("Couldn't serialize user feedback.")
            let itemHeader = SentryEnvelopeItemHeader(type: SentryEnvelopeItemTypeUserFeedback, length: 0)
            self.init(header: itemHeader, data: Data())
        }
    }
    #endif // !SDK_V9
    ...
    @objc public convenience init?(attachment: Attachment, maxAttachmentSize: UInt) {
        ...
        let itemHeader = SentryEnvelopeAttachmentHeader(
            type: SentryEnvelopeItemTypeAttachment,
            length: finalData.count,
    ...
    @objc public convenience init(clientReport: SentryClientReport) {
        do {
            ...
            let itemHeader = SentryEnvelopeItemHeader(type: SentryEnvelopeItemTypeClientReport, length: json.count)
            self.init(header: itemHeader, data: json)
        } catch {
            SentrySDKLog.error("Couldn't serialize client report.")
            let itemHeader = SentryEnvelopeItemHeader(type: SentryEnvelopeItemTypeClientReport, length: 0)
            self.init(header: itemHeader, data: Data())
        }
    }
    ...
    @objc public convenience init?(replayEvent: SentryReplayEvent, replayRecording: SentryReplayRecording, video: URL) {
        ...
        guard let recording = SentrySerialization.data(withReplayRecording: replayRecording) else {
        ...
        let itemHeader = SentryEnvelopeItemHeader(type: SentryEnvelopeItemTypeReplayVideo, length: UInt(envelopeItemContent.count))
        self.init(header: itemHeader, data: envelopeItemContent)
    }
    
    // MARK: - Private Helpers
    
    private func envelopeType(for event: Event) -> String {
        if event.type == SentryEnvelopeItemTypeTransaction {
            return SentryEnvelopeItemTypeTransaction
        } else if event.type == SentryEnvelopeItemTypeFeedback {
            return SentryEnvelopeItemTypeFeedback
        } else {
            return SentryEnvelopeItemTypeEvent
        }
    }
}
