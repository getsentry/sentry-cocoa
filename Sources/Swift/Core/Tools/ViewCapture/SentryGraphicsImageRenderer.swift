...
class SentryGraphicsImageRenderer {
    struct Context {
        ...
        var currentImage: UIImage {
            guard let cgImage = cgContext.makeImage() else {
                SentryLogSwift.fatal("Unable to create image from graphics context")
                return UIImage()
            }
    ...
    func image(actions: (Context) -> Void) -> UIImage {
        ...
        guard let rawData = calloc(pixelsPerColumn * bytesPerRow, MemoryLayout<UInt8>.size) else {
            SentryLogSwift.error("Unable to allocate memory for image data")
            return UIImage()
        }
        defer {
        ...
        guard let context = CGContext(
            ...
            SentryLogSwift.error("Unable to create context")
            return UIImage()
        }
