                                     ...
                                     (NSError *_Nullable *_Nullable)error
{
    ...
    if (nil != event.json) {
        ...
        [SentryLog
            logWithMessage:
                @"Using event->json attribute instead of serializing event"
    ...
    if ([SentrySDK.currentHub getClient].options.logLevel
        == kSentryLogLevelVerbose) {
        [SentryLog
            logWithMessage:@"Sending JSON -------------------------------"
                  andLevel:kSentryLogLevelVerbose];
        [SentryLog
    ...
    return [self initStoreRequestWithDsn:dsn
                                 andData:jsonData
                                     ...
                                     (NSError *_Nullable *_Nullable)error
{
    ...
    if (self) {
        ...
        self.HTTPBody = [data sentry_gzippedWithCompressionLevel:-1
                                                           error:error];
    }
    return self;
}
                                        ...
                                        (NSError *_Nullable *_Nullable)error
{
    ...
    if (self) {
        ...
        self.HTTPMethod = @"POST";
        [self setValue:authHeader forHTTPHeaderField:@"X-Sentry-Auth"];
        [self setValue:@"application/x-sentry-envelope"
    ...
    if ([SentrySDK.currentHub getClient].options.logLevel
        ...
        [SentryLog
            logWithMessage:
                [NSString stringWithFormat:@"Envelope request with data: %@",
...
newAuthHeader(NSURL *url)
{
    ...
    [string appendFormat:@"%@,",
            newHeaderPart(@"sentry_version", SentryServerVersionString)];
    [string appendFormat:@"%@,",
            newHeaderPart(@"sentry_client",
                [NSString stringWithFormat:@"sentry-cocoa/%@",
                          SentryMeta.versionString])];
    [string appendFormat:@"%@,",
            newHeaderPart(@"sentry_timestamp",
                @((NSInteger)[[NSDate date] timeIntervalSince1970]))];
    [string appendFormat:@"%@", newHeaderPart(@"sentry_key", url.user)];
    if (nil != url.password) {
        [string
            appendFormat:@",%@", newHeaderPart(@"sentry_secret", url.password)];
    }
    return string;
}
