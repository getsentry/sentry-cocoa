                             ...
                             configuration:(SentryTracerConfiguration *)configuration;
{
    ...
    SENTRY_LOG_DEBUG(@"Initializing tracer");
    ...
    SENTRY_LOG_DEBUG(@"Getting view names");
    ...
    SENTRY_LOG_DEBUG(@"Finished getting view names");

#endif // SENTRY_HAS_UIKIT

    _dispatchTimeoutLock = [[NSObject alloc] init];
    if ([self hasIdleTimeout]) {
        SENTRY_LOG_DEBUG(@"Starting idle timeout");
        [self startIdleTimeout];
    }

    if ([self isAutoGeneratedTransaction]) {
        SENTRY_LOG_DEBUG(@"Starting deadline timeout");
    ...
    SENTRY_LOG_DEBUG(@"Getting frame numbers");
...
- (void)finish
{
    SENTRY_LOG_DEBUG(@"Finishing tracer with id %@", self.traceId.sentryIdString);
