                             ...
                             configuration:(SentryTracerConfiguration *)configuration;
{
    ...
    [hub configureScope:^(SentryScope *scope) {
        if (scope.currentScreen != nil) {
            self->viewNames = @[ (NSString *_Nonnull)scope.currentScreen ];
        }
    }];
...
#if SENTRY_TARGET_PROFILING_SUPPORTED
    NSDate *_Nullable startTimestamp = self.startTimestamp;
    if (startTimestamp == nil) {
        // TODO: Handle this case
    }
    sentry_stopProfilerDueToFinishedTransaction(_hub, _dispatchQueue, transaction, _isProfiling,
        (NSDate *_Nonnull)startTimestamp, _startSystemTime
#    if SENTRY_HAS_UIKIT
        ,
        appStartMeasurement
#    endif // SENTRY_HAS_UIKIT
...
- (BOOL)finishTracer:(SentrySpanStatus)unfinishedSpansFinishStatus shouldCleanUp:(BOOL)shouldCleanUp
{
    ...
    NSTimeInterval transactionDuration =
        [self.timestamp timeIntervalSinceDate:(NSDate *_Nonnull)startTimestamp];
    if ([self isAutoGeneratedTransaction]
    ...
    @synchronized(_children) {
        for (id<SentrySpan> childSpan in _children) {
            if (childSpan.timestamp != nil &&
                [oldest compare:(NSDate *_Nonnull)childSpan.timestamp] == NSOrderedAscending) {
