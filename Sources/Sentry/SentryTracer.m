...
- (BOOL)finishTracer:(SentrySpanStatus)unfinishedSpansFinishStatus shouldCleanUp:(BOOL)shouldCleanUp
{
    ...
    NSTimeInterval transactionDuration = [self.timestamp timeIntervalSinceDate:SENTRY_UNWRAP_NULLABLE(NSDate, self.startTimestamp)];
    if ([self isAutoGeneratedTransaction]
...
- (void)trimEndTimestamp
{
    ...
    @synchronized(_children) {
        for (id<SentrySpan> childSpan in _children) {
            if (childSpan.timestamp && [oldest compare:SENTRY_UNWRAP_NULLABLE(NSDate, childSpan.timestamp)] == NSOrderedAscending) {
    ...
    "double-checked lock produce false alarms")
{
    ...
    NSTimeInterval difference = [appStartEndTimestamp timeIntervalSinceDate:SENTRY_UNWRAP_NULLABLE(NSDate, self.startTimestamp)];
