...
namespace profiling {
    ...
    ThreadHandle::all() noexcept
    {
        ...
        if (SENTRY_LOG_KERN_RETURN(task_threads(mach_task_self(), &list, &count))
            == KERN_SUCCESS) {
    ...
    ThreadHandle::name() const noexcept
    {
        ...
        if (SENTRY_LOG_ERRNO_RETURN(pthread_getname_np(handle, name, sizeof(name))) == 0) {
    ...
    ThreadHandle::priority() const noexcept
    {
        ...
        if (SENTRY_LOG_ERRNO_RETURN(pthread_getschedparam(handle, nullptr, &param)) == 0) {
