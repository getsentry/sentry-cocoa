...
namespace profiling {
    ...
    ThreadHandle::dispatchQueueAddress() const noexcept
    {
        ...
        thread_info(handle_, THREAD_IDENTIFIER_INFO, info, &count);
        ...
        //        if (rv != MACH_SEND_INVALID_DEST && SENTRY_PROF_LOG_KERN_RETURN(rv) ==
        //        KERN_SUCCESS
        //            && sentrycrashmem_isMemoryReadable(idInfo, sizeof(*idInfo))) {
        //            const auto queuePtr = reinterpret_cast<dispatch_queue_t
        //            *>(idInfo->dispatch_qaddr); if (queuePtr != nullptr &&
