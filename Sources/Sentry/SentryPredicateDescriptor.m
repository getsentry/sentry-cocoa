...
- (NSString *)compoundPredicateDescription:(NSCompoundPredicate *)predicate {
    ...
    if (expressions.count == 1) {
        return [NSString stringWithFormat:@"%@ %@",[self compoundPredicateTypeDescription:predicate.compoundPredicateType], expressions.firstObject];
    }
    
    return [expressions componentsJoinedByString:[self compoundPredicateTypeDescription:predicate.compoundPredicateType]];
}
...
- (NSString *)expressionDescription:(NSExpression *)predicate {
    switch (predicate.expressionType) {
        case NSConstantValueExpressionType: return @"%@";
        case NSAggregateExpressionType:
            if ([predicate.collection isKindOfClass:[NSArray class]]) {
                __block NSMutableArray * itens = [[NSMutableArray alloc] initWithCapacity:[predicate.collection count]];
        ...
        default: return predicate.description;
    }
}

- (NSString *)compoundPredicateTypeDescription:(NSCompoundPredicateType)compountType {
    switch (compountType) {
        case NSAndPredicateType: return @" AND ";
        case NSOrPredicateType: return @" OR ";
        case NSNotPredicateType: return @"NOT";
        default: return @", ";
    }
}
