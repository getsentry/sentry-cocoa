...
#    define SENTRY_PROF_LOG_KERN_RETURN(statement)                                                                     \
        ({                                                                                                             \
            ...
            if (__log_kr != KERN_SUCCESS) {                                                                            \
                SENTRY_PROF_LOG_ERROR("%s failed with kern return code: %d, description: %s", #statement, __log_kr,    \
...
#    define SENTRY_PROF_LOG_MACH_MSG_RETURN(statement)                                                                 \
        ({                                                                                                             \
            ...
            if (__log_mr != MACH_MSG_SUCCESS) {                                                                        \
                SENTRY_PROF_LOG_ERROR("%s failed with mach_msg return code: %d, description: %s", #statement,          \
                    __log_mr, sentry::profiling::machMessageReturnCodeDescription(__log_mr));                          \
            }                                                                                                          \
            __log_mr;                                                                                                  \
        })
