 ...
 * @note Provided for use by profiling, which does not use @c NSDate as @c SentryTransaction does for its normal timestamps. @c NSDate is not a safe API for time-sensitive measurements, because the time can be changed in iOS settings, or by the system when it recalibrates with a remote source of truth. The system time is based on the CPU clock.
 */
