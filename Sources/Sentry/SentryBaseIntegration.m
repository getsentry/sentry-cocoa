...
{
    [SentryLog logWithMessage:[NSString stringWithFormat:@"Not going to enable %@ %@.", self.integrationName, reason]
...
- (BOOL)shouldBeEnabledWithOptions:(SentryOptions *)options
{
    ...
    if ((integrationOptions & kIntegrationOptionEnableAutoSessionTracking) && !options.enableAutoSessionTracking) {
        [self logWithOptionName:@"enableAutoSessionTracking"];
        return NO;
    }

    if ((integrationOptions & kIntegrationOptionEnableOutOfMemoryTracking) && !options.enableOutOfMemoryTracking) {
    ...
    if ((integrationOptions & kIntegrationOptionEnableNetworkTracking) && !options.enableNetworkTracking) {
        [self logWithOptionName:@"enableNetworkTracking"];
        return NO;
    }

    if ((integrationOptions & kIntegrationOptionEnableFileIOTracking) && !options.enableFileIOTracking) {
        [self logWithOptionName:@"enableFileIOTracking"];
        return NO;
    }

    if ((integrationOptions & kIntegrationOptionEnableNetworkBreadcrumbs) && !options.enableNetworkBreadcrumbs) {
        [self logWithOptionName:@"enableNetworkBreadcrumbs"];
        return NO;
    }

    if ((integrationOptions & kIntegrationOptionEnableCoreDataTracking) && !options.enableCoreDataTracking) {
