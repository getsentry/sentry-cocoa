...
namespace internal {
     ...
     * function.
     */
    bool isCppExceptionKillswitchSet(
        const std::string &fnName = "", const std::string &file = "", int line = 0) noexcept;

    /**
     * Runs a lambda, handling exceptions encountered and then setting the kill switch if one
     * occurs.
     *
     ...
     * @param file                  Source filename of the calling function, used for logging
     * exceptions
     * @param line                  Source line of the calling function, used for logging exceptions
     */
    template <typename Function1, typename Function2>
    void
    handleCppException(Function1 &&func, Function2 &&bailIfSwitchSetFunc,
        const std::string &fnName = "", const std::string &file = "", int line = 0) noexcept
    {
        try {
            ...
            SPECTO_LOG_CRITICAL("Exception raised in function {} [{}:{}]: {}", fnName, file, line,
                exception.what());
            /**
...
#    define SPECTO_HANDLE_CPP_EXCEPTION_IF_ALIVE(fn, fn2)                                          \
        _SPECTO_HANDLE_CPP_EXCEPTION_IF_ALIVE(fn, fn2)
