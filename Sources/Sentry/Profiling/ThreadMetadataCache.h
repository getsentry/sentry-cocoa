...
/**
 * Caches thread metadata (name, priority, etc.) and prepopulates a recycled `proto::Entry` object
 * with that data. A new `proto::Entry` and corresponding `proto::Backtrace` object are created
 * for each thread, if one does not already exist.
 *
