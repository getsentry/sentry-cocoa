 ...
 *
 * @param kr The mach message return code to get a description for.
 * @return A string containing the description, or an unknown error message if
 * the error code is not known.
 */
...
#define SPECTO_LOG_KERN_RETURN(statement)                                            \
    ({                                                                               \
        ...
        if (__log_kr != KERN_SUCCESS) {                                              \
            SPECTO_LOG_ERROR("{} failed with kern return code: {}, description: {}", \
                             #statement,                                             \
                             __log_kr,                                               \
                             specto::darwin::kernelReturnCodeDescription(__log_kr)); \
        }                                                                            \
        __log_kr;                                                                    \
    })
    ...
    ({                                                                                    \
        ...
        if (__log_mr != MACH_MSG_SUCCESS) {                                               \
            SPECTO_LOG_ERROR("{} failed with mach_msg return code: {}, description: {}",  \
                             #statement,                                                  \
                             __log_mr,                                                    \
                             specto::darwin::machMessageReturnCodeDescription(__log_mr)); \
        }                                                                                 \
        __log_mr;                                                                         \
    })
