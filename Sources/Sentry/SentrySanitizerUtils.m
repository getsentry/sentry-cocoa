...
static NSArray *sentry_sanitize_array_with_depth(NSArray *array, NSUInteger depth);
    ...
    NSDictionary *_Nullable dictionary, NSUInteger depth)
{
    ...
    for (id rawKey in dictionaryCopy.allKeys) {
        ...
        if ([rawValue isKindOfClass:NSString.class]) {
            ...
            [dict setValue:sentry_sanitize_array_with_depth(rawValue, depth + 1) forKey:stringKey];
...
NSDictionary *_Nullable sentry_sanitize_dictionary(NSDictionary *_Nullable dictionary)
{
    return sentry_sanitize_with_depth(dictionary, 0);
}

static NSArray *
sentry_sanitize_array_with_depth(NSArray *array, NSUInteger depth)
{
    if (depth >= kMaxSanitizeDepth) {
        return @[];
    }
    ...
    for (id rawValue in arrayCopy) {
        if ([rawValue isKindOfClass:NSString.class]) {
            ...
            [result addObject:sentry_sanitize_array_with_depth(rawValue, depth + 1)];
        } else if ([rawValue isKindOfClass:NSDate.class]) {
            NSDate *date = (NSDate *)rawValue;
...
NSArray *
sentry_sanitize_array(NSArray *array)
{
    return sentry_sanitize_array_with_depth(array, 0);
}
