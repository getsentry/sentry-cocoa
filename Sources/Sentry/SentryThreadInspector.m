...
unsigned int getStackEntriesFromThread(SentryCrashThread thread, struct SentryCrashMachineContext *context, SentryCrashStackEntry * buffer, unsigned int amount) {
    ...
    @synchronized(self) {
        ...
        for (int i = 0 ; i<numSuspendedThreads; i++) {
            if (suspendedThreads[i] != currentThread) {
                int numberOfEntries = getStackEntriesFromThread(suspendedThreads[i], context, threadsInfos[i].stackEntries, 100);
        ...
        for (int i = 0; i < numSuspendedThreads; i++) {
            ...
            if (isCurrent) {
                ...
                sentryThread.stacktrace = [self.stacktraceBuilder buildStackTraceFromStackEntries:threadsInfos[i].stackEntries amount:threadsInfos[i].stackLength];
            }
