...
/**
 * Fetch all subclasses of UIViewController from given objc Image on a background thread and then act on them on the main
 * thread. As there is no straightforward way to get all sub-classes in Objective-C, the code first
 * retrieves all classes from the Image, iterates over all classes, and checks for every class if
 * the parentClass is a UIViewController. Cause loading all classes can take a few milliseconds, do this on a
 * background thread.
 *
