...
- (NSDictionary<NSNumber *, NSDate *> *)parse:(NSString *)header
{
    ...
    for (NSString *quota in
        [headerNoWhitespaces componentsSeparatedByString:@","]) {
        NSArray<NSString *> *parameters =
            [quota componentsSeparatedByString:@":"];

        NSNumber *rateLimitInSeconds =
            [self parseRateLimitSeconds:parameters[0]];
...
- (NSArray<NSNumber *> *)parseCategories:(NSString *)categoriesAsString
{
    ...
    for (NSString *categoryAsString in
        [categoriesAsString componentsSeparatedByString:@";"]) {
        SentryRateLimitCategory category =
            [self mapStringToCategory:categoryAsString];
...
{
    NSDate *newDate = [SentryCurrentDate.date
