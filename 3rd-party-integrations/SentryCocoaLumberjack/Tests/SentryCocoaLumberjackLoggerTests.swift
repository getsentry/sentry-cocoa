...
final class SentryCocoaLumberjackLoggerTests: XCTestCase {
    ...
    func testFiltering_WithInfoThreshold_FiltersDebugAndVerbose() {
        // -- Arrange --
        let sut = getSut()
        DDLog.add(sut, with: .info)
        ...
        // -- Assert --
        XCTAssertEqual(capturedLogs.count, 3, "Expected 3 logs (info, warning, error) to be captured")
        
        let levels = capturedLogs.map { $0.level }
        XCTAssertTrue(levels.contains(.info), "Expected info log to be captured")
        XCTAssertTrue(levels.contains(.warn), "Expected warning log to be captured")
        XCTAssertTrue(levels.contains(.error), "Expected error log to be captured")
        XCTAssertFalse(levels.contains(.debug), "Debug log should be filtered out")
        XCTAssertFalse(levels.contains(.trace), "Verbose log should be filtered out")
    }
    
    func testFiltering_WithErrorThreshold_OnlyCapturesErrors() {
        // -- Arrange --
        let sut = getSut()
        DDLog.add(sut, with: .error)
        ...
        // -- Assert --
        XCTAssertEqual(capturedLogs.count, 1, "Expected 1 log (error) to be captured")
        XCTAssertEqual(capturedLogs.first?.level, .error, "Expected error log to be captured")
        XCTAssertEqual(capturedLogs.first?.body, "Error message")
    }
    
    func testFiltering_WithVerboseThreshold_CapturesAllLevels() {
        // -- Arrange --
        let sut = getSut()
        DDLog.add(sut, with: .verbose)
        ...
        // -- Assert --
        XCTAssertEqual(capturedLogs.count, 5, "Expected all 5 logs to be captured")
        
        let levels = capturedLogs.map { $0.level }
        XCTAssertTrue(levels.contains(.trace), "Expected verbose log to be captured")
        XCTAssertTrue(levels.contains(.debug), "Expected debug log to be captured")
        XCTAssertTrue(levels.contains(.info), "Expected info log to be captured")
        XCTAssertTrue(levels.contains(.warn), "Expected warning log to be captured")
        XCTAssertTrue(levels.contains(.error), "Expected error log to be captured")
    }
