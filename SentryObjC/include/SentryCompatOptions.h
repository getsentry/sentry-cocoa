...
@property (nonatomic, strong, nullable) SentryCompatDsn *parsedDsn;

/// Block called for every captured event before it's sent. Return nil to drop.
@property (nonatomic, copy, nullable) SentryCompatEvent *_Nullable (^beforeSend)(SentryCompatEvent *event);

/// Block called for every span before it's sent. Return nil to drop.
@property (nonatomic, copy, nullable) SentryCompatSpan *_Nullable (^beforeSendSpan)(SentryCompatSpan *span);

/// Block called for every breadcrumb before it's added. Return nil to drop.
@property (nonatomic, copy, nullable) SentryCompatBreadcrumb *_Nullable (^beforeBreadcrumb)(SentryCompatBreadcrumb *breadcrumb);

/// Called once after init with the crash event captured during the previous run.
@property (nonatomic, copy, nullable) void (^onCrashedLastRun)(SentryCompatEvent *event)
...
@property (nonatomic, copy, nullable)
    void (^onLastRunStatusDetermined)(SentryCompatLastRunStatus status, SentryCompatEvent *_Nullable event);

/// Dynamic sampler for transactions.
@property (nonatomic, copy, nullable)
    NSNumber *_Nullable (^tracesSampler)(SentryCompatSamplingContext *context);

/// Decide whether to capture a screenshot for a given event.
@property (nonatomic, copy, nullable) BOOL (^beforeCaptureScreenshot)(SentryCompatEvent *event);

/// Decide whether to capture a view hierarchy for a given event.
@property (nonatomic, copy, nullable) BOOL (^beforeCaptureViewHierarchy)(SentryCompatEvent *event);

/// Block used to construct the initial scope. Defaults to identity.
@property (nonatomic, copy) SentryCompatScope *(^initialScope)(SentryCompatScope *scope);
