...
final class SentryCrashWrapperTests: XCTestCase {
    ...
    func testEnrichScope_WithSystemInfo_SetsOSContext() {
        crashWrapper.enrichScope(scope)
        
        let osContext = scope.contextDictionary["os"] as? [String: Any]
        XCTAssertNotNil(osContext)
        XCTAssertNotNil(osContext?["name"])
        XCTAssertNotNil(osContext?["version"])
    ...
    func testEnrichScope_WithSystemInfo_SetsDeviceContext() {
        crashWrapper.enrichScope(scope)
        
        let deviceContext = scope.contextDictionary["device"] as? [String: Any]
    ...
    func testEnrichScope_WithSystemInfo_SetsAppContext() {
        crashWrapper.enrichScope(scope)
        
        let appContext = scope.contextDictionary["app"] as? [String: Any]
    ...
    func testEnrichScope_DeviceContext_ContainsSimulatorFlag() {
        crashWrapper.enrichScope(scope)
    ...
    func testEnrichScope_DeviceContext_ContainsDeviceFamily() {
        crashWrapper.enrichScope(scope)
    ...
    func testEnrichScope_RuntimeContext_MacCatalyst() {
        if #available(iOS 13.0, macOS 10.15, watchOS 6.0, tvOS 13.0, *) {
            #if targetEnvironment(macCatalyst)
            crashWrapper.enrichScope(scope)
            
            let runtimeContext = scope.contextDictionary["runtime"] as? [String: Any]
            XCTAssertNotNil(runtimeContext)
