...
- (void)test_getScreenshots_CheckName
{
    ...
    SentryCrash* sentryCrash = [[SentryCrash alloc] initWithBasePath:[self.tempPath stringByAppendingPathComponent:@"Reports"]];
    NSArray *files = [sentryCrash getScreenShots:12];
    
    XCTAssertEqualObjects(files.firstObject, [self.tempPath stringByAppendingPathComponent:@"Reports/AppName-report-000000000000000c-screenshots/0.png"]);
...
{
    SentryCrash* sentryCrash = [[SentryCrash alloc] initWithBasePath:[self.tempPath stringByAppendingPathComponent:@"ReportsFake"]];
    NSArray *files = [sentryCrash getScreenShots:12];
    XCTAssertEqual(files.count, 0);
}
...
- (void)initReport:(uint64_t)reportId withScreenshots:(int)amount {
    ...
    [NSFileManager.defaultManager createDirectoryAtPath:ssDir withIntermediateDirectories:true attributes:nil error:nil];
    
    for (int i = 0; i < amount; i++) {
        NSString *name = [NSString stringWithFormat:@"%i.png", i];
        [[name dataUsingEncoding:NSUTF8StringEncoding] writeToFile:[ssDir stringByAppendingPathComponent:name] atomically:YES];
    }
}
