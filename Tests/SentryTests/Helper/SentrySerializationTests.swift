...
class SentrySerializationTests: XCTestCase {
    ...
    private class Fixture {
        ...
        static var traceContext = SentryTraceContext(trace: SentryId(), publicKey: "PUBLIC_KEY", releaseName: "RELEASE_NAME", environment: "TEST", transaction: "Some Transaction", user: SentryTraceContextUser(userId: "User Id", segment: "some segment"), sampleRate: NSNumber(value: 0.25))
    }
    ...
    func testSentryEnvelopeSerializer_TraceStateWithoutUser() {
        let trace = SentryTraceContext(trace: SentryId(), publicKey: "PUBLIC_KEY", releaseName: "RELEASE_NAME", environment: "TEST", transaction: "Some Transaction", user: nil, sampleRate: nil)
