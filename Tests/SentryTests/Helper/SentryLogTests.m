...
/**
 * Test that when setting the diagnostic level to each value in @c SentryLevel and then receiving log
 * messages at all levels, that only the levels at or above the configured level are processed.
 */
...
+ (void)initialize {
    ...
    for (SentryLevel l = kSentryLevelDebug; l < kSentryLevelFatal; l++) {
        ...
        [self sentry_addInstanceMethodWithSelectorName:name testLogic:^(__unused XCTestCase * _Nonnull testCase) {
            SentryTestLogOutput *logOutput = [[SentryTestLogOutput alloc] init];
            ...
            NSArray *expected = [allExpectedMessages objectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(l, kSentryLevelFatal - l + 1)]];
            NSArray *actual = logOutput.loggedMessages;
            XCTAssert([expected isEqualToArray:actual], @"Expected %@ but got %@", expected, actual);
        }];
    }
}
