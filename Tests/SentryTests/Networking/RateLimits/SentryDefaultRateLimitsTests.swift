...
class SentryDefaultRateLimitsTests: XCTestCase {
    ...
    /// category. This covers the "header on a successful response" case, sent lowercase over HTTP/2.
    ...
    /// The case-insensitive header scan compares keys case-insensitively, so even an unconventional
    /// mixed casing resolves to the correct category instead of falling through to the generic 429
    /// handling that rate-limits everything.
    func testUpdate_whenMixedCaseRateLimitHeaderOver429_shouldOnlyRateLimitGivenCategory() throws {
        // -- Arrange --
        let response = try XCTUnwrap(HTTPURLResponse(
            url: URL(fileURLWithPath: ""),
            statusCode: 429,
    ...
    /// A 200 response without any rate-limit header must not rate-limit anything. Covers the branch
    /// where the case-insensitive header lookup finds no matching header (returns nil).
    func testUpdate_whenNoRateLimitHeaderOn200_shouldNotRateLimitAnything() throws {
        // -- Arrange --
        let response = try XCTUnwrap(HTTPURLResponse(
            url: URL(fileURLWithPath: ""),
            statusCode: 200,
        ...
        // -- Assert --
        XCTAssertFalse(sut.isRateLimitActive(SentryDataCategory.error))
        XCTAssertFalse(sut.isRateLimitActive(SentryDataCategory.replay))
    }
