...
class SentryCodableTests: XCTestCase {
    ...
    func testDecodeWithEmptyData_ReturnsNil() {
        XCTAssertNil(decodeFromJSONData(jsonData: Data()) as GeoDecodable?)
    }
    ...
    func testDecodeWithWrongJSON_ReturnsEmptyObject() {
        ...
        let actual = decodeFromJSONData(jsonData: wrongJSON) as GeoDecodable?
        let expected = Geo()
    ...
    func testDecodeWithBrokenJSON_ReturnsNil() {
        ...
        XCTAssertNil(decodeFromJSONData(jsonData: brokenJSON) as GeoDecodable?)
    }
