...
- (void)testConvertReport
{
    ...
    XCTAssertEqualObjects(
        [NSDate dateWithTimeIntervalSince1970:@(1491210797).integerValue],
        event.timestamp);
    XCTAssertEqual(event.debugMeta.count, (unsigned long)256);
    SentryDebugMeta *firstDebugImage = event.debugMeta.firstObject;
    XCTAssertTrue([firstDebugImage.name
        isEqualToString:@"/var/containers/Bundle/Application/"
                        @"94765405-4249-4E20-B1E7-9801C14D5645/"
                        @"CrashProbeiOS.app/CrashProbeiOS"]);
    XCTAssertTrue([firstDebugImage.uuid
        isEqualToString:@"363F8E49-2D2A-3A26-BF90-60D6A8896CF0"]);
    XCTAssertTrue(
        [firstDebugImage.imageAddress isEqualToString:@"0x0000000100034000"]);
    XCTAssertTrue(
        [firstDebugImage.imageVmAddress isEqualToString:@"0x0000000100000000"]);
    XCTAssertEqualObjects(firstDebugImage.imageSize, @(65536));

    SentryException *exception = event.exceptions.firstObject;
    XCTAssertEqualObjects(
        exception.thread.stacktrace.frames.lastObject.symbolAddress,
        @"0x000000010014c1ec");
    XCTAssertEqualObjects(
        exception.thread.stacktrace.frames.lastObject.instructionAddress,
        @"0x000000010014caa4");
    XCTAssertEqualObjects(
        exception.thread.stacktrace.frames.lastObject.imageAddress,
        @"0x0000000100144000");
    XCTAssertEqualObjects(
        exception.thread.stacktrace.registers[@"x4"], @"0x0000000102468000");
    XCTAssertEqualObjects(
        exception.thread.stacktrace.registers[@"x9"], @"0x32a77e172fd70062");
    ...
    NSString *code =
        [NSString stringWithFormat:@"%@",
                  [exception.mechanism.meta valueForKeyPath:@"signal.code"]];
    NSString *number =
        [NSString stringWithFormat:@"%@",
                  [exception.mechanism.meta valueForKeyPath:@"signal.number"]];
    NSString *exc = [NSString
        stringWithFormat:@"%@",
        [exception.mechanism.meta valueForKeyPath:@"mach_exception.name"]];
    XCTAssertEqualObjects(code, @"0");
    XCTAssertEqualObjects(number, @"10");
    XCTAssertEqualObjects(exc, @"EXC_BAD_ACCESS");
    XCTAssertEqualObjects(
        [exception.mechanism.data valueForKeyPath:@"relevant_address"],
        @"0x0000000102468000");
...
- (void)testRawWithCrashReport
{
    ...
    NSDictionary *eventJson =
        [self getCrashReport:@"Resources/converted-event"];

    NSArray *convertedDebugImages
        = ((NSArray *)[eventJson valueForKeyPath:@"debug_meta.images"]);
...
- (void)testNXPage
{
    ...
    SentryException *exception = event.exceptions.firstObject;
    XCTAssertEqualObjects(
        exception.thread.stacktrace.frames.lastObject.function, @"<redacted>");
}
...
- (void)testReactNative
{
    ...
    XCTAssertEqualObjects(
        event.exceptions.firstObject.value, @"SentryClient: Test throw error");
...
{
    [self isValidReport:
              @"Resources/sentry-ios-cocoapods-report-0000000053800000"];
...
- (void)testFatalError
{
    ...
    SentryCrashReportConverter *reportConverter =
        [[SentryCrashReportConverter alloc] initWithReport:rawCrash];
    SentryEvent *event = [reportConverter convertReportToEvent];
    XCTAssertEqualObjects(event.exceptions.firstObject.value,
        @"crash: > fatal error > hello my crash is here");
}
...
- (void)testBreadCrumb
{
    ...
    SentryEvent *event = [reportConverter convertReportToEvent];
    XCTAssertEqualObjects(
        event.breadcrumbs.firstObject.category, @"ui.lifecycle");
    ...
    XCTAssertEqualObjects(
        [event.breadcrumbs.firstObject.data objectForKey:@"screen"],
...
{
    XCTAssertEqual([one allKeys].count, [two allKeys].count,
        @"one: %@, two: %@", one, two);
    for (NSString *key in [one allKeys]) {
        if ([[one valueForKey:key] isKindOfClass:NSString.class] &&
            ...
            [[NSScanner scannerWithString:[one valueForKey:key]]
                scanHexLongLong:&result1];
            [[NSScanner scannerWithString:[two valueForKey:key]]
                scanHexLongLong:&result2];
            ...
            for (NSUInteger i = 0; i < oneArray.count; i++) {
                [self compareDict:[oneArray objectAtIndex:i]
                         withDict:[twoArray objectAtIndex:i]];
            }
        } else if ([[one valueForKey:key] isKindOfClass:NSDictionary.class]) {
            [self compareDict:[one valueForKey:key]
                     withDict:[two valueForKey:key]];
        } else {
...
{
    NSString *jsonPath =
        [[NSBundle bundleForClass:self.class] pathForResource:path
                                                       ofType:@"json"];
    NSData *jsonData =
        [NSData dataWithContentsOfURL:[NSURL fileURLWithPath:jsonPath]];
...
{
    NSData *jsonData =
        [NSJSONSerialization dataWithJSONObject:[event serialize]
