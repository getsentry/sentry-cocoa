...
- (void)testAddRequest {
    ...
    SentryEvent *event = [[SentryEvent alloc] initWithMessage:@"bayoom" timestamp:[NSDate date] level:kSentrySeverityDebug];
...
- (void)testRealRequest {
    ...
    SentryEvent *event = [[SentryEvent alloc] initWithMessage:@"bayoom" timestamp:[NSDate date] level:kSentrySeverityDebug];
    [client sendEvent:event withCompletionHandler:^(NSError * _Nullable error) {
        [expectation fulfill];
    }];
    [self waitForExpectationsWithTimeout:5.0 handler:^(NSError *error) {
        if (error) {
