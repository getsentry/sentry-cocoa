...
struct NetworkEnvelopeBaseline {
    static func assertMatches(
        ...
        assertContains(expected: expected, actual: actual, file: file, line: line)
    }
    ...
    private static func assertContains(
        ...
        if let expected = expected as? [String: Any] {
            guard let actual = actual as? [String: Any] else {
                return XCTFail("Expected dictionary, got \(String(describing: actual)).", file: file, line: line)
            }
            for (key, value) in expected {
                assertContains(expected: value, actual: actual[key], file: file, line: line)
            }
            return
        }
        ...
        if let expected = expected as? [Any] {
            guard let actual = actual as? [Any] else {
                return XCTFail("Expected array, got \(String(describing: actual)).", file: file, line: line)
            }
            XCTAssertEqual(actual.count, expected.count, file: file, line: line)
            for (expectedValue, actualValue) in zip(expected, actual) {
                assertContains(expected: expectedValue, actual: actualValue, file: file, line: line)
            }
            return
        }
