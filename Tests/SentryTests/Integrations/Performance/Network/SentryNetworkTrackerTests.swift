...
class SentryNetworkTrackerTests: XCTestCase {
    ...
    /// Regression test for #8388: the response `Content-Type` used to be read via a case-sensitive
    /// `allHeaderFields[@"Content-Type"]` subscript. HTTP/2 and HTTP/3 lowercase field names, so a
    /// server sending `content-type` made the lookup return nil, and the captured body was not
    /// parsed (it degraded to a `[Body not captured: contentType=unknown …]` placeholder). The
    /// lookup is now case-insensitive, so the JSON body is parsed even with a lowercased header.
    func testCaptureResponseDetails_withLowercasedContentType_parsesJSONBody() throws {
        ...
        // The server sends the header lowercased, as HTTP/2 and HTTP/3 require. We can't build this
        // with the public initializer because it canonicalizes well-known headers back to
        // `Content-Type`, so we use a subclass that returns the wire casing from `allHeaderFields`.
        ...
        // Before the fix, the case-sensitive lookup returned nil, so the body degraded to the
        // "[Body not captured: contentType=unknown …]" placeholder string instead of parsed JSON.
        let parsedBody = try XCTUnwrap(
            bodyDict["body"] as? [String: Any],
            "Response body should be parsed as JSON even when Content-Type is lowercased"
        )
    ...
    /// reproduce #8388, where the case-sensitive `allHeaderFields["Content-Type"]` subscript missed
    /// a lowercased `content-type`.
