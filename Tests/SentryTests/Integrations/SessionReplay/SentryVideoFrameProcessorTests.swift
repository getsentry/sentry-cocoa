...
class SentryVideoFrameProcessorTests: XCTestCase {
    ...
    func testProcessFrames_WhenInputIsReady_ShouldProcessFrames() {
        let sut = fixture.getSut()
    ...
    func testProcessFrames_WhenInputNotReady_ShouldNotProcessFrames() {
        let sut = fixture.getSut()
        let videoWriterInput = TestAVAssetWriterInput(mediaType: .video, outputSettings: nil)
        videoWriterInput.isReadyForMoreMediaDataOverride = false
    ...
    func testProcessFrames_WhenImageCannotBeLoaded_ShouldSkipFrame() {
        ...
        let nonExistentFrames = [
            SentryReplayFrame(imagePath: "/non/existent/path.png", time: Date(), screenName: "Screen1"),
        ...
        XCTAssertEqual(sutWithNonExistentFrames.frameIndex, 2)
