...
Test applications used as UI test hosts and manual acceptance testing.

Most subdirectories contain an xcodeproj per platform/version, with one or more targets describing variants of applications targeting it, and including the Sentry SDK via a local Swift package reference (`packages:` in the XcodeGen spec). Each app's xcodeproj is included in the top level Sentry.xcworkspace.

## Project Structure

- Uses `xcodegen` with YAML configuration files for test apps
- Test app configurations in `TestApps/*/[AppName].yml`
- Shared configurations and code in `TestApps/SentrySampleShared/`
- Run `make xcode` after checkout to ensure projects are up-to-date

## SDK Configuration

The iOS-Swift and iOS-ObjectiveC test apps have schema launch args and environment variables available to customize how the SDK is configured.
...
To help pinpoint where to look to debug an issue we see on our Sentry dashboard for the `sentry-sdks` project for events coming from our test apps, each test app injects some information from the build environment automatically.
...
Therefore the test apps use manual code signing, see [fastlane docs](https://docs.fastlane.tools/codesigning/xcode-project/):
...
You can run test apps in a real device without changing certificates and provisioning profiles if you are a Sentry employee with access to Sentry [profiles repository](https://github.com/getsentry/codesigning) and 1Password account.
