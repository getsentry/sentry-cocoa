...
This test app demonstrates **using the Sentry SDK from Objective-C++ when `-fmodules` and `-fcxx-modules` are not supported**.
...
```bash
cd TestApps/iOS-ObjectiveCpp-NoModules
xcodegen generate
```
...
```bash
make build-testapp-iOS-ObjectiveCpp-NoModules
```
