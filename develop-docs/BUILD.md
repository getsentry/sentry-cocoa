...
There are two build configurations they can use for this: `DebugWithoutUIFramework` and `ReleaseWithoutUIFramework`, that are essentially the same as `Debug` and `Release` with the following differences:
