...
| `replayId: String?`                                  | Scope replay ID (V9); active replay ID in all modes (V10) |
| `isBuffering: Bool`                                  | Whether the active replay is buffering (V10 only)         |
...
>
> [!WARNING]
> In V10, `replayId` reads the active replay integration, so a non-`nil` value no longer implies that a full-session replay is running or that a buffered replay was captured. Hybrid SDKs upgrading to V10 must also read `isBuffering`. When it is `true`, do not write the ID to scope or treat replay capture as successful; when enriching telemetry, attach `sentry._internal.replay_is_buffering = true`. V9 and the legacy `PrivateSentrySDKOnly.getReplayId` remain scope-based and do not return buffered replay IDs.
...
In V10, hybrid SDKs can associate telemetry with an active buffered replay:
