...
`updateSessionForDroppedEventNonTerminating(unhandled:)` applies the same session side effects without sending an envelope. Hybrid SDKs should call it when an error is dropped by sampling, so the native session still records the error. Do not call it for events dropped by `beforeSend` or ignored exception types.
